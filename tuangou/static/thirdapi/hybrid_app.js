...
function connectWebViewJavascriptBridge(callback) {
  ...
  if (!(ua.indexOf('ICBCiPhoneBS') > -1 || ua.indexOf('ICBCAndroidBS') > -1)) {
    // 工银e生活
    return ;
  }
  ...
  if (window.WebViewJavascriptBridge) {
    ...
    console.log('-------------connectWebViewJavascriptBridge----------add bridge----------')
...
hybrid_app.getDefaultAddress = function (custId, appId) {
  if (ICBCUtilTools.isAndroid()) {
    hybrid_app.GetNativeFunctionAndroid({
      'keyword': 'getDefaultAddress',
      'custId': custId,
      'appId': appId,
