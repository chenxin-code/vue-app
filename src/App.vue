  ...
  <div id="app" class="theme_sys_base" :class="theme">
    ...
    <transition :name="$router.customRouterData.transitionName">
      <keep-alive ref="keep_alive" key="key_keepalive">
        <router-view :key="$route.path" class="router_class" v-if="$route.meta.keepAlive"/>
      </keep-alive>
    </transition>
    <transition :name="$router.customRouterData.transitionName">
      <router-view :key="$route.path" class="router_class" v-if="!$route.meta.keepAlive"/>
    ...
    <div
      ...
      <transition-group :name="homeBtnSeat == 'right' ? 'backIndexBtn_r' : 'backIndexBtn_l'">
        <div class="div-shy" v-show="isShy == true" key="backIndexBtn_shy" @click.stop="dontShy">
...
<script>
  ...
  export default {
    ...
    mounted() {
      ...
      if (this.$store.state.login.token && this.$store.state.deployType == '2' && this.$store.state.webtype == '1' && this.$store.state.globalConfig.isShowHbOilMessageBox == 1) {
    ...
    methods: {
      oilIsOpen() {
        http.post('/app/json/app_youdi/getAcctInfoList', {
          token: this.$store.state.login.token
        }).then(res => {
          let data = res.data;
          if (data.status == 0) {
            if (data.data.length == 0 || !data.data || (data.data && data.data.state == 0)) {
              /**
               * 获取弹框次数
               */
              http.post('/app/json/user/getPopTimes', {
                token: this.$store.state.login.token
              }).then(res => {
                let data = res.data;
                if (data.status == 0) {
                  if ((data.data && Number(data.data) < 2) || !data.data) {
                    MessageBox.confirm('油惠通诚邀您开通油滴账户，开通后加油购物均可赠送油滴。油滴可以抵现，也可以换购其他优惠哦~', '提示', {
                     ...
                     * 弹框一次 增加一次次数
                     */
                    http.post('/app/json/user/savePopTimes', {
                      token: this.$store.state.login.token
                    }).then(res => {
                    })
                  }
      ...
      showBackBtn(routePathArrs) {
        ...
        if (this.$store.state.webtype == "3") {
          if (routePathArrs.length > 0) {
            ...
            return !(lastRoute == "/live-stream/list" ||
        ...
        if (routePathArrs.length >= 3) {
          let lastRoute = routePathArrs[routePathArrs.length - 1];
          return !(lastRoute == "/usercenter/login" ||
