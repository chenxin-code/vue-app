<template>
  <div class="payorder">
    <!-- header -->
    <nav-top title="加油支付凭证" @backEvent="backEvent"></nav-top>
    <nav-content class="firm-order-content">
      ...
      <div class="order-list"
        ...
        <div class="order-item"  v-for="(item, index) in orderList" @click="repayDetail(item)" :key="index">
          <div class="item">
            <span class="label">贵阳南明体育馆加油站</span>
            <span class="con">2020年10月13日11:07:45</span>
          </div>
          <div class="item">
          ...
          <div class="item pay-btn">
            ...
            <div v-if="tabActive == '0'">
              <div class="con wait-pay again" @click.stop="repayment(item)">重新支付</div>
              <div class="con wait-pay" style="margin-right:15px" @click.stop="cancelOrderHandler(item)">取消订单</div>
            </div>
            <div class="con" v-else>
              <div class="con wait-pay" @click.stop="toDetail(item)">查看订单</div>
            </div>
          </div>
        </div>
    ...
    <!--   </div>-->
    <!-- </van-popup>-->
<!--    <repayments @queryOrder="queryOrder" :isScan="isScan" ref="repay"></repayments>-->
  </div>
</template>
<script>
  ...
  export default {
    ...
    methods: {
      ...
      repayment(item) {
        ...
        this.$router.push({path: '/offline_payment/confirm_bill', query:{source: this.source}})
      },
      shareView() {
      ...
      getPayBill() {
        ...
        let params = {
          token: this.$store.state.login.token,
          orderTypeList: ["100011", "100002002", "100027002"], // 100011 app加油 100002002 扫码加油
          rows: 12,
          page: this.page,
        }
        if (this.tabActive == 1) {
