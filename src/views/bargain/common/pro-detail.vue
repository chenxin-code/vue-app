...
<!--        </div>-->
    ...
    <div class="bottom-btns" v-if="detailShow && justdetail != '1'">
...
<!--        &lt;!&ndash;<i class="iconfont mall-shoucang btn-icon theme_font_red" v-if="isCollect == 'true'"></i>&ndash;&gt;-->
<!--        <transition :name="collectName">-->
<!--          <img class="btn-img" src="static/image/mall2/collect.png" v-if="isCollect == 'true'">-->
<!--        </transition>-->
<!--        <p class="text">收藏</p>-->
<!--      </div>-->
      <div class="btn theme_bg_y theme_font_white" style="width: 100%" @click="buyNowEvent"
           v-if="stockNum != 0 && canSale == true && detailData.status != '0' && isPayTime()&&$store.state.globalConfig.cut_price_strict!=1">
        <p class="title">¥{{oprice}}购买</p>
      </div>
      <div class="btn theme_standard_bg theme_font_white" style="width: 100%" @click="kanjiana"
           v-if="stockNum != 0 && canSale == true && detailData.status != '0'">
        <p class="title">¥{{kprice}}砍价拿</p>
      </div>
      <!--<div class="btn big-btn theme_bg_y theme_font_white" @click="noStockEvent" v-if="detailData.status">-->
      <!--<p class="title">'商品已下架'</p>-->
      <!--</div>-->
      <div class="btn big-btn theme_bg_y theme_font_white" style="width: 100%" @click="noStockEvent"
           v-if="stockNum == 0 && canSale == true && detailData.status != '0'">
        <p class="title">商品缺货</p>
      </div>
      <div class="btn big-btn theme_bg_y theme_font_white" style="width: 100%" @click="canNotSaleEvent"
           v-if="canSale == true && detailData.status == '0'">
        <p class="title">商品已下架</p>
      </div>
      <div class="btn big-btn theme_bg_y theme_font_white" style="width: 100%" @click="canNotSaleEvent" v-if="canSale == false">
    ...
    <pop-view v-if="showPop" @closeEvent="popClose">
      <div class="pop-body">
        ...
        <div class="bottom-btn theme_font_white theme_bg_red" @click="popSureEvent"
             v-if="popFlag == 'addToCart' && stockNum != 0 && canSale == true && detailData.status != '0'">加入购物车
        </div>
...
<script>
  ...
  import ShareImage from '@/components/mall2/detail/shareImage'
  ...
  export default {
    ...
    methods: {
      ...
      launchBtn() { // 发起 productId：参与活动商品id，bargainActId：活动id
        this.$request.post("/app/json/user_bargain/startBargainAct",{origId: this.$route.query.origId, productId: this.$route.query.id, bargainActId: this.$route.query.bargainActId}).then(res => {
          if (res.status == 0) {
            this.cutPriceBtn(res.data.id)
          } else {
            this.$Toast(res.info);
          }
        })
      },
      cutPriceBtn(id) { // 砍价 origUserId：发起人用户ID，role：角色（0:自砍，1:帮砍）productId：帮砍商品id
      console.log(this.$route.query)
        this.$request.post("/app/json/user_bargain/cutPrice",{origId: id, productId: this.$route.query.id, origUserId: this.userInfo.userId, role: 0}).then(res => {
          if (res.status == 0) {
            let path = '/bargain/bargain_details'
            this.$router.replace({
              path: path,
              query: Object.assign({},this.$route.query,{origId: id})
            })
          } else {
            this.$Toast(res.info);
          }
      ...
      getScrollTop: function () {
        if (this.justdetail == '1') {
          return 'top: ' + this.$store.state.barHeight + 'px;' + 'bottom: 0px;';
        }
    ...
    created() {
      this.kprice = this.$route.query.kprice ? this.$route.query.kprice : ''
      this.oprice = this.$route.query.oprice ? this.$route.query.oprice : ''
      this.justdetail = this.$route.query.justdetail ? this.$route.query.justdetail : ''
      ...
      this.bargainOrderId = this.$route.query.bargainOrderId || ''

      console.log(this.id,this.skuId)
