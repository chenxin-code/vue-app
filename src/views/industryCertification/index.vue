  ...
  <div class="box">
    ...
    <nav-content>
      <ul class="card" v-if="authentication.length">
        <scroll ref="scroll" :hasMore="hasMore" :pullUpLoad="true"  @pullingUp="onLoad" :data="authentication">
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.$request.post(this.industryClusterUrl, {page: this.page,rows: this.rows,}).then(res => {
        this.funStatus = true
        let data = res.data;
        if (res.status == 0) {
          this.authentication = this.authentication.concat(data || [])
          if (this.authentication.length >= res.totalRecords) {
            this.hasMore = false;  
          }
        } else {
          this.$Toast(res.info)
