        ...
        <div class="certificates">
          <div class="certificatesItem">
            <div class="imgBox" @click="takePictures(3,'imageUrl3')">
              <img class="backgroundImg" :src="travel.imageUrl3 || 'static/image/vocational-certification/rx@2x.png'" alt="">
              <img class="tipsImg" src="static/image/vocational-certification/icon1@2x.png" alt="">
            </div>
            <div class="text">
      ...
      <div v-for="(item,index) in idNameList" :key="index">
        ...
        <div class="certificatesBox">
          <div class="certificates">
            <div class="certificatesItem">
              <div class="imgBox">
                <img class="backgroundImg" :src="item.newCredImg" alt="">
              </div>
              <div class="text">
                <p>查看示意图</p>
              </div>
      ...
      <van-popup v-model="popupDrivingCard" :close-on-click-overlay="false" class="examine">
        <div class="popupContent">
          <van-cell-group>
            ...
            <van-field v-model="travel.plateNum" label="车牌号" input-align="right" :readonly='true'/>
...
export default {
  ...
  mounted() {
    this.$nextTick(() => {
      this.qualificationsObj = this.activeContent
      ...
      if (this.qualificationsObj.isCertificate) { // 身份
        Object.assign(this.identityObj, this.mergeObj(this.qualificationsObj.usercertificate, this.identityObj))
        this.status.identityObj = true
      }
      if (this.qualificationsObj.isDrinving) { // 驾驶证
        Object.assign(this.drive, this.mergeObj(this.qualificationsObj.driverInfoModel, this.drive))
        this.status.drive = true
      }
      if (this.qualificationsObj.isVehicle) { // 行驶证
        Object.assign(this.travel, this.mergeObj(this.qualificationsObj.vehicleModel, this.travel))
  ...
  methods: {
    ...
    ocrCertificate(type, base64StrImg, activeObjType, dataUrl) {
      ...
      //                 imgFront       imgBehind     imageUrl    imageUrl2     imageUrl3
    ...
    getIdentityObj(isActiveObj,active){
      ...
      if (this.activeContent.activeObj.type == 'IDcard' ) {
        ...
        if (!this.status.travel) {
          ...
          this.travel.imageUrl = this.travel.imageUrl3
      ...
      } else if (this.activeContent.activeObj.type == 'examine') {
        this.submmitIndustryFun(true)
      }
    },
    routerPush(){
      this.isExamine = true
    ...
    popupIDcardBtn() {
      ...
      this.$request.post(this.identifySubmmitUrl, this.identityObj).then(res => {
        if (res.status === 0) {
          this.$Toast('提交成功')
          eventBus.$emit('isActive',this.isActiveObjNew, 'true', 'identityObj')
        } else {
          this.$Toast(res.info)
        }
      })
    ...
    popupDriverCardBtn() {
      ...
      this.$request.post(this.driverInfoSubmmitUrl, this.drive).then(res => {
        if (res.status === 0) {
          this.$Toast('提交成功')
          eventBus.$emit('isActive',this.isActiveObjNew, 'true', 'drive')
        } else {
          this.$Toast(res.info)
        }
      })
    ...
    popupDrivingCardBtn() {
      ...
      this.$request.post(this.bindVehicleUrl, this.travel).then(res => {
        if (res.status === 0) {
          this.$Toast('提交成功')
         eventBus.$emit('isActive',this.isActiveObjNew, 'true', 'travel')
        } else {
          this.$Toast(res.info)
        }
      })
    },
    submmitIndustryFun(status) {
      this.qualifications.clusterId = this.activeContent.id
      this.qualifications.clusterCode = this.activeContent.clusterCode
      this.qualifications.clusterName = this.activeContent.clusterName
      this.$request.post(this.submmitIndustryUrl, this.qualifications).then(res => {
