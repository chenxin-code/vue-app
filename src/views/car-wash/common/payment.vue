  ...
  <div>
    ...
    <nav-content class="container">
      <div class="block-content prod-info">
        <img src="../images/icon-wash.png" alt class="prod-img"/>
        <div class="right">
          <p class="name">{{ paramsData.skuName }}</p>
          <div class="price">
            <span class="current-price">¥{{ $util.toDecimal2(paramsData.salePrice) }}</span>
            <span class="buy-num">x{{ paramsData.carts[0].number }}</span>
          </div>
        </div>
      </div>
      <div class="bottom-btn">
        <div class="left-price">
          <!-- 应付金额：<span class="num">￥{{ newPrice !== undefined ? newPrice : dealWithPrice($route.params.occur.payAmount)[2] }}</span> -->
        </div>
        <div class="chum-public-btn submit-btn">
          <van-button round block type="info" color="linear-gradient(270deg,rgba(49,49,49,1) 0%,rgba(84,84,84,1) 51%,rgba(49,49,49,1) 100%)" @click="submitOrder">提交订单
          </van-button>
        </div>
      </div>
    </nav-content>
    <van-popup
      ...
      <!-- <p class="price-c">￥<span class="num">{{ newPrice !== undefined ? newPrice : dealWithPrice($route.params.occur.payAmount)[2] }}</span></p> -->
  ...
  export default {
    ...
    data() {
      return {
        occurData: {},
        paramsData: {},
        // purchaseNum: this.$route.params.paramsData.carts[0].number,
    ...
    mounted() {
      // this.occurData = this.$route.query.occur || {}
      this.paramsData = this.$route.query.paramsData || {}
      console.log(this.$route.query.occur, this.$route.query.paramsData)
    ...
    methods: {
      // 立即支付
      confirmBuyNow() {
        payHelper.payEvent(this.selectedPayWay, this.submitData.orderType, this.submitData.orderId, `/carWash/scanPaySuccess?state=3&entryType=${this.entryType}&orderType=${this.submitData.orderType}&orderId=${this.submitData.orderId}`).then(() => {
        }).catch(() => {
          this.$Toast('cancel')
        })
      },
