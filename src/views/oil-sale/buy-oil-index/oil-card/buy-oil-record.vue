<template>
  <div class="have-mention">
    <nav-top class="nav-top" title="购油记录" @backEvent="$router.go(-1)">
      <div class="screen-popup">
        <ScreenPopup :isShowChangeCard="isShowChangeCard" @childToDadValue='screenChild'></ScreenPopup>
      </div>
    </nav-top>
    <nav-content class="have-mention-container">
      <div class="my-title-info" v-for="(item, index) in cardList" :key="index" v-if="complete">
        <div class="top">
          <div class="card-num">油品名称：{{item.oilcode}}{{item.oilname}}</div>
        </div>
        <div class="list">
          <div class="item">
...
<script>
  ...
  export default {
    data() {
      return {
        cardList: [],
        isShowChangeCard: false, // 购油记录 不能筛选卡号
        startTime: '',
        endTime: '',
        complete: false,
        hasMore: true
    ...
    methods: {
      ...
      getRecord() {// 发送请求获取数据
        ...
        this.$request.post('/app/json/app_oil_appoint/appAllOilSaledRecord', formDataJson)
          .then(res => {
            this.complete = true
            this.hasMore = false
            if (res.status === 0) {
              ...
              this.$Toast(res.info ? res.info : '获取购油记录失败')
            }
          }).catch(err => {
          console.log(err);
        })
