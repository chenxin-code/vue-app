<template>
  <div class="oil-sale-container">
    <nav-top title="抢单成功" @backEvent="$router.go(-1)"></nav-top>
              ...
              <div class="table-view-cell-text">
                <div class="bill-item">
                  ...
                  <div class="center-text">
                    ...
                    <div class="time">抢单时间：{{resultItem.submitTime}}</div>
                    <div class="tips info">
                      ￥<span class="now-price">{{resultItem.price ? resultItem.price : 0}}.00</span>/t
                      <span class="default-num">￥{{resultItem.basePrice ? resultItem.basePrice : 0}}</span>
                    </div>
                  </div>
                </div>
              </div>
...
<script>
  ...
  export default {
    ...
    methods: {
      dealNum() {
        let res = (this.resultItem.basePrice * this.resultItem.tonnes).toFixed(2)
        return (Math.round(res * 100) / 100).toFixed(2)
      },
      calculatePrice() {
        if (this.resultItem.basePrice > this.resultItem.price) {
          let num1 = this.resultItem.basePrice * this.resultItem.tonnes
          let num2 = this.resultItem.price * this.resultItem.tonnes
