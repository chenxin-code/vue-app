<template>
  <div class="cart">
    ...
    <nav-content>
      ...
      <div :class="{notop: $store.state.globalConfig.delivertype_default == '1' || $store.state.globalConfig.delivertype_default == '2'}">
        <p style="margin: 10px;font-size: 15px;">普通商品</p>
         <CartList v-for="(item,index) in topArr" :ref="getCartRef(item.type)" v-if="item.hasShow"
                  v-show="deliverType == item.type" :deliverType="item.type" :key="index" :isEditing="isEditing"
                  @updateEdit="setEdit">
        </CartList>
        <p style="margin: 10px;font-size: 15px;">团购商品</p>
        <buyingCartList v-for="(item,index) in topArr" :ref="getCartRef(item.type)" v-if="item.hasShow"
                  v-show="deliverType == item.type" :deliverType="item.type" :key="index+100" :isEditing="isEditing"
                  @updateEdit="setEdit">
        </buyingCartList>
      </div>
    </nav-content>
...
<script>
  ...
  import buyingCartList from './buyingCart.vue'
