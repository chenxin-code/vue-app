...
<script>
    ...
    import CartItem from './buying-item'
    ...
    export default {
        ...
        methods: {
                ...
                if (this.isEditing) {
                    ...
                    arr.forEach((element,index) => {
                        if(element.deliverType == this.deliverType && element.skuId == cartitem.skuId){
                            this.$store.state.microSho.groupbuyingCarts[index].checked = this.$store.state.microSho.groupbuyingCarts[index].checked ? 0 : 1
                            return
                        }
                    })
                    this.getDataList();
            ...
            setOccSel: function (occuritem) { // 顶级 勾选
                if (this.isEditing) {
                    ...
                    for (let j = 0; j < occuritem.store.length; j++) {
                        ...
                        let arr = this.getSelStore(storeitem,false);
                        selArr = selArr.concat(arr);
                    }
                    let arr = this.$store.state.microSho.groupbuyingCarts
                    selArr.forEach((res,resIndex)=>{
                        arr.forEach((brx,index) => {
                            if(brx.skuId == res.skuId && brx.deliverType == this.deliverType) {
                                this.$store.state.microSho.groupbuyingCarts[index].checked = occuritem.checked ? 0 : 1
                            }
                        })
                    })
                    this.getDataList();
            ...
            //获取所有商铺下面商品的集合
            getSelStore: function (storeitem,status) {
                let selArr = [];
                ...
                let cartitem = storeitem.noActivityCart[activityIndex];
                    selArr.push({
                        checked: status ? cartitem.checked : (cartitem.checked ? 0 : 1),
                        deliverType: cartitem.deliverType,
                        number: cartitem.number,
                        proSkuDistributionId: cartitem.proSkuDistributionId,
                        selfActivityId: cartitem.selfActivityId,
            ...
            toPay (occuritem) {
                ...
                for (let j = 0; j < occuritem.store.length; j++) {
                    let storeitem = occuritem.store[j];
                    let arr = this.getSelStore(storeitem ,true);
                    selArr = selArr.concat(arr);
                }
                let cartsNew = []
                selArr.forEach(res=>{
                    if (res.checked) {
                        carts.forEach(item=>{
                            if (item.skuId == res.skuId) {
                                res.proSkuDistributionId = item.proSkuDistributionId
                                res.selfActivityId = item.selfActivityId
                            }
                        })
                        cartsNew.push(res)
                    }
            ...
            toDelete: function (occuritem) {
                let arr = JSON.parse(JSON.stringify(this.$store.state.microSho.groupbuyingCarts))
                    ...
                    res.noActivityCart.forEach(item=>{
                        arr.forEach((brx,index) => {
                            if(brx.deliverType == this.deliverType && item.skuId == brx.skuId){
                                this.$store.state.microSho.groupbuyingCarts.splice(index ,1)
                            }
                        })
                    })
                })
            ...
            numChange: function (itemInfo) { // 数量
                ...
                arr.forEach((element,index) => {
                    if(itemInfo.skuId == element.skuId){
                        this.$store.state.microSho.groupbuyingCarts[index].number = itemInfo.number
                        return
                    }
                })
                this.getDataList();
            ...
            getDataList () {
                ...
                if(newAll.length){
                    this.$Loading.open();
                    let url = '/app/json/app_purchase_order/getCart';
                    let paramsData = {
                        token: this.$store.state.login.token,
                        microStoreCode: this.microStoreCode,
                        carts: newAll,
        ...
        created() {
            ...
            console.log(1,this.$route.query.lastPath)
            // if(this.$route.query.lastPath == '/pay-success') {
