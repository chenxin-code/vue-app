<template>
  <div class="coupon-list">
      ...
      <div class="coupon-content" v-if="couponMenuListIndex == 0">
        ...
        <ul class="sub-menu-list" v-show="curType === unusedValue">
          ...
          <horizontal-scroll :dataList='couponTypeList'>
            ...
            <li class="sub-menu-item" :class="{active: curCateAlias === 'gouwuquan'}"  @click="subMenuHandler('gouwuquan')">
              <span class="text" :class="{'theme_standard_bdr_i theme_standard_font_i theme_light_bg_i': curCateAlias === 'gouwuquan'}">
                购物券
                ({{couponPackageLength}})
              </span>
            </li>
        ...
        <div class="coupon-list-wrapper" :class="{'no-padding': !hasBtmBtn}" v-show="hasGetData">
          <scroll ref="scroll" class="coupon-list-scroll" :listenScroll="true" :probe-type="3" @scroll="scroll"
            ...
            <template>
              <!-- 券包展示 -->
              <template v-if="curCateAlias === 'gouwuquan' && curType == 'getUnusedCouponsWithCols'">
                <couponPackage class="coupon-item" v-for="(item, index) in couponList" :key='index' :item='item'></couponPackage>
              </template>
              <template v-else>
                <coupon :couponLayoutType="isGroupCoupon ? 2 : 1" v-for="(item, index) in couponList" :disabled="disabled"
                      ...
                      class="coupon-item" @load="reset"></coupon>
              </template>
        ...
        <div class="footer-btn" v-if="hasBtmBtn">
          ...
          <a @click='viewQrcode' class="btn">查看会员二维码</a>
        </div>
      </div>
    ...
    <van-dialog v-model="showQrcode" title="会员二维码" confirmButtonText='关闭'>
      <div class="qr-warp" style='text-align: center;'>
...
<script type="text/ecmascript-6">
  ...
  import couponPackage from './base/coupon-package'
  ...
  export default {
    ...
    data() {
      return {
        ...
        scrollY: 0,
        _scrollY: 0,
        showQrcode: false, //会员二维码展示
        couponPackageLength: 0, //券包数量
    ...
    methods: {
      ...
      viewQrcode() {
        ...
        // 渲染会员二维码
        this.$nextTick(()=>{
          QRCode.toCanvas(document.getElementById('qrcode'),this.$store.state.userInfo.userId || '', {width: 180})
        })
      },
      getCouponPackage() {
        // 获取券包详情
        this.$http.post("/app/json/coupon/getUnusedCouponsGroupByPackage",{
          token: this.$store.state.login.token
        }).then(res=>{
          let data = res.data;
          if(data.status == 0) {
            this.couponPackageLength = data.totalRecords || 0;
            if(Array.isArray(data.data) && data.data.length) {
              // 支持后端，目前后端有一个没有couponPackage的券包，总数减1
              data.data.forEach(item=>{
                if(!item.couponPackage) {
                  this.couponPackageLength = data.totalRecords -1;
                }
              })
            }
          } else {
      ...
      loadMore(isReset) {
        ...
        if (this.curType === this.unusedValue && this.curCateAlias && this.curCateAlias != 'quanbu' && this.curCateAlias !== 'gouwuquan') {
          paramsData.couTypeCategoryAlias = this.curCateAlias
        }
        let url = `/app/json/coupon/${this.curType}`;
        // 如果是购物券，券包形式的
        if(this.curCateAlias === 'gouwuquan' && this.curType == 'getUnusedCouponsWithCols') {
          url = '/app/json/coupon/getUnusedCouponsGroupByPackage'
        }
        return this.$http.post(url, paramsData).then(res => {
          let data = res.data
          if (data.status == 0) {
            if(!Array.isArray(data.data) || !data.data.length) {
              this.$Loading.close()
            ...
            // 非购物券的一些操作
            if(this.curCateAlias !== 'gouwuquan') {
              let curMenu = null
              let num = 0
      ...
      getCouTypeCateCnt() {
        ...
        this.$http.post(`/app/json/coupon/getCouTypeCateCnt`, params).then(res => {
          ...
          if (data.status == 0) {
            ...
            this.curCateAlias = list[0].cateAlias;
    ...
    components: {Coupon, HorizontalScroll, Scroll,couponPackage}
  }
</script>
