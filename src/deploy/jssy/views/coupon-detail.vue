  ...
  <div>
    ...
    <nav-content class="coupon-detail-container" v-if="$util.objectKeys(couponDetail).length">
      <div class="coupon-content">
        <coupon @load="getMyCouInfo"  :couponLayoutType="isGroupCoupon ? 2 : 1" :user="5" :hasBtn="false"  :item="couponDetail"></coupon>
      </div>
      <!--等于0是待激活  充值券不显示二维码-->
      <!--商城券不显示二维码-->
      <!--couponQrCodeEnabled全局配置项，1 为展示二维码（充值券默认是隐藏）-->
      <div class="qr-code-wrapper" v-if="showQrcode && couponDetail.couOriginType == 20">
        <h3>易捷(江苏)便利店二维码</h3>
            <canvas class="qr-code" id="hxqrcode"></canvas>
    <!--        海信券等于20展示 编码-->
      </div>
      <div class="qr-code-wrapper" v-if="showQrcode">
        <h3>优惠券二维码</h3>
        ...
        <!-- <div class="refresh-wrapper" v-else  @click="getMyCouInfo">
          <i class="iconfont mall-shuaxin"></i>
      ...
      couponRequest: function (imgVerifyCode, uuid) {
        ...
        this.$http.post(`/app/json/coupon/getMyCouInfo`, paramsData).then(res => {
          ...
          if (data.status == 0) {
            ...
            if(this.couponDetail.couOriginType) {
               this.$nextTick(()=>{
                 QRCode.toCanvas(document.getElementById('hxqrcode'),this.$store.state.userInfo.userId || '', {width: 240})
               })
            }
            this.$nextTick(() => {
