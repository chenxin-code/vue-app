...
<script>
// import TopNav from "../../base/top-nav";
// import NavContent from "../../base/nav-content";
import JsBarcode from "jsbarcode";
export default {
  name: "myself",
  // components: { NavContent, TopNav },
  data() {
    return {
      item: "",
      code: "",
      ...
      this.$http
        ...
        .then(res => {
          if (res.data.status === 0) {
            ...
            if (data) {
              this.coucode = data.coucode;
              this.facevalue = data.facevalue;
              this.tradeno = data.tradeno;
              this.tradeorg = data.tradeorg;
              this.epsno = data.epsno;
              this.usedtime = data.usedtime;
              this.code = data.coucode;
              this.$nextTick(() => {
                this.createBar(data.coucode);
              });
            }
            this.$Loading.close();
          }
