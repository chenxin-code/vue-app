            ...
            <ul class="invitation-list">
              <li class="invitation-item" v-for="(item, index) in orderDetailModelList" :key="index">
                ...
                <div class="oil-right" v-else @click="deleteBtn(item)">
                  <img src="../../../views/oil-package/image/icon-delete.png" alt="">
                </div>
              </li>
            </ul>
            <!--            成员不足 分享按钮-->
  ...
  export default {
    ...
    methods: {
      share() {
        ...
        console.log(`${location.origin}${Config.shareUrl}/redirect?shareId=${this.details.id}`)
        Jwx.share(
          this.details.packageRefuelName,
          `${location.origin}${Config.shareUrl}/redirect?shareId=${this.details.id}`,
          this.details.pic,
      ...
      signPackageRefuel() {
        ...
        if (this.$route.query.startDate) {
          time = this.$route.query.startDate
        } else {
          time = utils.getNextMinutes()
        }
        let inviteelds = []
