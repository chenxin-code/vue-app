<template>
  <div>
        ...
        <div class="info">
          <div class="pro-info">
            ...
            <div class="media-content">
              <div class="name">
                <span class="group-num">{{groupDetail.groupBuyNumOfPerson}}人团</span>
                {{groupDetail.skuName}}
              </div>
          ...
          <div class="group-state">
            ...
            <div class="state-sub-msg"
                 v-if="myGroupDetail.state == undefined || myGroupDetail.state == 2 || myGroupDetail.state == 4">
              <span class="text">还差{{groupDetail.groupBuyNumOfPerson - groupDetail.currentNumOfPerson}}人！距结束</span>
            ...
            <div class="member-list">
              <div class="member-item" v-for="(member, idx) in groupDetail.orderItemList"
                   v-if="idx < groupDetail.groupBuyNumOfPerson">
                <div class="member-avatar-wrapper">
                  <div class="member-avatar">
                    <img class="img" :src="member.userAvatar ? member.userAvatar : 'static/images/defaultImg.png'" mode="widthFix" alt="">
                  </div>
                  <span class="tit" v-if="idx == 0">团长</span>
                </div>
                <div class="member-name ellipsis single-line">{{member.userNickName}}</div>
              </div>
            ...
            <div class="btn" v-if="myGroupDetail.state == undefined && groupDetail.state < 6" @click="toAddGroup">立即参团</div>
            <div class="btn" v-else-if="myGroupDetail.state == 2" @click="sharegoods">邀请好友</div>
            <div class="btn" v-else-if="myGroupDetail.state == 4" @click="toPay">去付款</div>
    ...
    <groupsuccess @closeEvent="isGroupSuccess = false" :successType="successType" @shareGroup="sharegoods" @toOrderList="toOrderList"
                  v-if="formPaySuccess != '0' && isGroupSuccess"></groupsuccess>
...
<script type="text/ecmascript-6">
  ...
  export default {
    ...
    computed: {
      ...
      residueNum() {
        return this.groupDetail.groupBuyNumOfPerson - this.groupDetail.currentNumOfPerson
      },
    },
    mounted() {
      this.orderId = this.$route.query.orderId
      this.mktGroupBuyId = this.$route.query.mktGroupBuyId
      this.formPaySuccess = this.$route.query.formPaySuccess ? this.$route.query.formPaySuccess : '0'
    ...
    methods: {
      ...
      setWxShareArgs: function () {
        ...
        if (this.$store.state.webtype == 2) {
          shareData.detailurl = this.$store.state.globalConfig.wxBaseUrl + Config.shareUrl + '/group_detail?orderId=' + this.orderId;
      ...
      queryGroupDetail() {
        ...
        this.$http.post('/app/json/app_fight_group_order/detail', paramsData).then(res => {
          let data = res.data
          this.$Loading.close()
          if (data.status == 0) {
            ...
            console.log(this.$store.state.userInfo.userId, this.groupDetail.leaderUserId)
            ...
            if (this.groupDetail.state != '9') {
              ...
              if (this.formPaySuccess == '1') {
                if (this.groupDetail.state == 10) {
                  this.successType = 3
                } else if (this.$store.state.userInfo.userId == this.groupDetail.leaderUserId) {
                  this.successType = 1
                } else {
                  this.successType = 2
                }
      ...
      queryAll() {
        ...
        this.$http.post('/app/json/app_fight_group_order/queryAll', paramsData).then(res => {
          let data = res.data
          if (data.status == 0) {
          } else {
            this.$Toast(data.info)
