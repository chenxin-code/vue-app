      ...
      <div ref="leftScroll" class="left-div theme_bg_white_f5" @scroll="scrollEvent">
        <div class="cat-div" :class="{theme_bg_white_i: selectedId == idx}" v-for="(item, idx) in category" @click="categoryEvent(idx)">
          <div class="title-div theme_font_gray"
               :class="{'theme_standard_bdr_i theme_font_black_i title-div-selected': selectedId == idx}">{{item.name}}
          </div>
        </div>
      </div>
        ...
        <div class="sub-right-div">
          <div class="block-div" v-for="(item, idx) in subCategories">
            <div class="title-row theme_font_black">
              ...
              <div class="more-btn theme_standard_font" v-if="item.clsType != 0" @click="toProductList(item, 2)">
                <span>全部</span>
                <i class="iconfont mall-gengduojiantou more-size"></i>
              </div>
            </div>
            <div class="s-cate" v-for="(subItem, cIndex) in item.childList" @click.stop="toProductList(subItem, 3)">
              <div class="cate-img">
  ...
  export default {
    ...
    methods: {
      ...
      categoryEvent: function (idx) {
        ...
        this.selectedId = idx;
        this.subDivKeywords = this.$market.getCategorySubPageKey(this.category[idx].id)
