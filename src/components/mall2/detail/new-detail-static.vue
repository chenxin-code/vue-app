...
<template>
  <div class="detail mall2">
                  ...
                  <div>
                    ...
                    <div class="base-info theme_bg_white">
                      ...
                      <div class="seconds-kill"
                           v-if="detailData.mktActivityPriceLevel == 3 || (showActivityTime == '1' && detailData.mktActivityPriceLevel != '')">
                        <div class="left-div">
                      ...
                      <!--限时特价-->
                      <div class="seconds-kill xstj" v-else-if="detailData.mktActivityPriceLevel === 1">
                        <div class="left-div">
                          <div class="icon-img">
                            <img src="./img/xstj.png">
                          </div>
                          <div class="price">
                            <p class="activity-price">活动价：¥<span>{{detailData.dpedData.integer}}</span>.{{detailData.dpedData.decimals}}</p>
                            <p class="orgin-price">原价：¥{{$util.toDecimal2(detailData.dpedData.linePrice)}}</p>
                          </div>
                        </div>
                        <div class="right-div">
                          <div class="skr-row theme_font_red">
                      ...
                      <!--贵州预售-->
                      <div class="seconds-kill ys" v-else-if="detailData.supportPreSale == 1 && $store.state.globalConfig.presaleType == 2">
                        <div class="left-div">
                          <div class="theme_font_white">
                      ...
                      <div class="row" v-if="noticeIsShow()">
                        <div class="activity-notice">
                          <div class="sk-icon">
                            <i class="iconfont mall-icon-test theme_bg_red theme_font_white"></i>
                            <span class="theme_font_red">{{detailData.noticeActivityDetail.activityType}}预告</span>
                          </div>
                    ...
                    <!--券和优惠活动-->
                    <div class="cou_and_act theme_bg_white"
                         v-if="(!cardType || (cardType && cardType == 551)) && orderCategory != '1'">
                      <CouponAndActivity :detailData="detailData" @openEvent="couponEvent"></CouponAndActivity>
                    </div>
                    ...
                    <!--预售信息-->
                    <div class="block-div theme_bg_white" v-if="detailData.supportPreSale == 1 && $store.state.globalConfig.presaleType == 2">
                      <div class="flex-row">
                    ...
                    <!--可参与的单品活动-->
                    <div v-if="singleActivities_2.length > 0 && orderCategory != '1'">
                      <div class="block-div theme_bg_white"
                    ...
                    <div class="block-div no-padding-bottom theme_bg_white" v-if="detailData.productType != 5">
                      ...
                      <!--</div>-->
                      <div class="color-row theme_bg_dark" @click="serversEvent"
                           v-if="$store.state.globalConfig.product_services_show_type != '0' && detailData.isShowAfterSale != 0">
                    ...
                    <div class="block-div theme_bg_white" v-if="detailComment && detailComment.evaluateCount > 0">
            ...
            <transition name="transition2">
              <div class="main-swiper" style="position: absolute; top: 44px;" v-show="proView == 2">
                <div class="btns-div theme_bg_white">
                  ...
                  <div class="btn-item line_left theme_font_gray"
                  ...
                  <div class="btn-item line_left theme_font_gray"
                       ...
                       @click="introductionSelected(3)"  v-if="detailData.isShowAfterSale != 0">售后包装
                  </div>
                </div>
                <div class="detail-div detail-div-1">
                  <pull-to ref="pullTo2"
                    ...
                    <div class="detail-content detail-bottom">
                      ...
                      <div v-html="introduction1" class="introduction-div" style="padding: 0" id="picMedia"
                           v-if="introductionIndex == 1"></div>
        ...
        <swiper-slide>
          <div class="main-swiper" style="position: relative;">
            <div class="main-swiper-scroll-content" style="top: 44px;">
              <Recommend ref="scrollView4" :deliveryType="this.deliveryType" :skuId="this.skuId" v-show="toptab == 4"
    ...
    <div class="bottom-tip theme_bg_yl theme_standard_font_i" v-if="canSale == false">该商品在该地区暂不支持购买，非常抱歉！</div>
    <div class="bottom-tip theme_bg_yl theme_standard_font_i" v-else-if="detailData.interfaceType == '2' || detailData.interfaceType == '1'">说明：图片中如显示价格仅做参考，以实际售价为准。</div>
    <div class="bottom-btns yj-card" v-if="productType == 8 || (easyCardId && !recommendCommodity)">
      <div class="btn theme_bg_red theme_font_white" :class="{'theme_bg_dark_d_i': canSale == false || detailData.status == 0}" @click="buyNowEvent">
    ...
    <div class="bottom-btns" v-else-if="!easyCardId && detailShow && !recommendCommodity">
      <div class="btn full" ref="gouwuche" @click="toCart">
        ...
        <template v-else>
          <p class="cart-num theme_bg_red theme_font_white" v-if="$store.state.mall2.cartNum > 0">{{$store.state.mall2.cartNum}}</p>
        </template>
      ...
      <div class="pop-body">
        ...
        <div class="pop-scroll-div">
          <div class="items-div" v-for="(feature, fidx) in detailData.featureList" v-if="featureRefresh">
            ...
            <div class="item theme_font_tint theme_font_common theme_bg_white_ef"
                 :class="{'theme_light_bg_i theme_standard_font_i theme_standard_bdr_i': subFeature.selected}" v-for="subFeature in feature.featureValueList" @click="subFeatureEvent(subFeature, fidx)">
              {{subFeature.featureValue}}
            </div>
          </div>
          <div class="count-line">
            <div class="full theme_font_common">购买数量<span v-if="$store.state.globalConfig.detailShowStockNum != 'false'">（库存 {{stockNum < 1000 ? stockNum : '充足'}}）</span></div>
            <!--<Counter v-model="selectedNum" :minValue="minNum" :stepNum="stepNum" :maxValue="maxNum"-->
            <!--@numChange="numChange"></Counter>-->
            <van-stepper v-model="selectedNum" :min="minNum" :max="maxNum" :step="stepNum" @blur="numChange" @change="generateSelectedText" integer/>
          </div>
        </div>
        <div class="bottom-btn theme_font_white theme_bg_red" @click="popSureEvent"
             v-if="popFlag == 'addToCart' && stockNum != 0 && canSale == true && detailData.status != '0' && (detailData.supportPreSale != '1' || $store.state.globalConfig.presaleType == 2) && detailData.productType != 8">
          加入购物车
        </div>
  ...
  export default {
    ...
    methods: {
      ...
      toCart: function () {
        ...
        if (this.detailData.supportPreSale == 1 && this.$store.state.globalConfig.presaleType == 2) {
          pushData = {
      ...
      couponEvent: async function () {
        if (this.detailData.couponInList == '' && this.jfhqList == '' && this.detailData.activityList.length == 0) {
      ...
      numChange: function () {
        ...
        // 如果不是整步幅，取整步幅
        num = parseInt((num - this.minNum) / this.stepNum) * this.stepNum + this.minNum
      ...
      _getProductDetail: function () {
        ...
        let storeOuCode = this.$route.query.storeOuCode ? this.$route.query.storeOuCode : '';
        ...
        if (deliverType == 1 || deliverType == '') {
          pickupStoreOuCode = this.$store.state.mall2.zitiAddress.storeCode ? this.$store.state.mall2.zitiAddress.storeCode : this.$route.query.storeOuCode ? this.$route.query.storeOuCode : '1';
        }
        ...
        staticDataRequest.request(url, funcName, args).then(data => {
          ...
          if (data.status == 0) {
            ...
            this.$actionStatistics.detailAction('120000', JSON.stringify(args), this.detailData.skuId)
            ...
            // 弥补后台bug
            if (this.detailData.orginDeliveryType == '1' || this.detailData.orginDeliveryType == '2') {
        ...
        this.$http.post(url, paramsData).then(
          res => {
            ...
            if (data.status == 0) {
              ...
              // 库存
              this.stockNum = data.data.skuStock.stockCurrentNumber > 0 ? data.data.skuStock.stockCurrentNumber : 0
              if (this.mktStockNum != -1) {
                // 活动库存，小于当前库存，取活动库存当库存
                this.stockNum = this.mktStockNum < this.stockNum ? this.mktStockNum : this.stockNum
              }
