...
<template>
  <div class="list">
    <div class="eccell" v-for="(item,index) in coupons" @click.stop="itemEvent(item,index)" :class="{'disable': item.disable == true}">
      <div class="ecinfo shadow-cell">
        <div class="left-info theme_font_white" :class="listType==4||listType==5||(selectedCouponType != '' && selectedCouponType != item.couTypeCode && listType == 2)?'theme_bg_dark_d':'theme_bg_light_blue'">
        ...
        <div class="right-info">
          <div class="right-con">
            ...
            <div class="coupon-tip">
              <div class="single-line font-small theme_font_tint" v-if="item.couCategory == 40">
                <!--{{item.couDiscountMaxValue ? `注：最大抵扣金额${item.couDiscountMaxValue}元` : ''}}-->
              </div>
              <div class="single-line font-small theme_font_tint" v-else-if="item.couFaceValue == ''">注：领取金额{{item.couMinFaceValue}}元至{{item.couMaxFaceValue}}元随机</div>
            </div>
            <div class="date-con">
              <div style="flex: 1;">{{item.validStartDate}} 至 {{item.validEndDate}}</div>
              <div class="btn get-coupon theme_bg_light_blue theme_font_white" v-if="item.isReceived == 0 && listType == 0" @click.stop="getCouponEvent(item)">点击领取</div>
              <div class="btn to-product theme_font_light_blue" v-if="item.isReceived == 1 && listType == 0" @click.stop="couponProducts(item)">可用商品</div>
              <div class="btn get-coupon theme_bg_light_blue theme_font_white" v-if="listType == 3" @click.stop="getCouponEvent(item)">立即使用</div>
            </div>
          </div>
        </div>
        <img src="/static/image/mall2/used.png" v-if="listType==4"/>
        <div class="select-div" v-if="listType == 2">
          <i class="iconfont mall-weixuanzhong theme_font_tint" v-if="!item.selected"></i>
          <i class="iconfont mall-xuanzhong theme_font_red" v-if="item.selected"></i>
        </div>
      </div>
    </div>
  </div>
