...
export default {
  open(options = {}) {
    ...
    if (instance.initData.type == 'use') {
      ...
      this.initCouponList(instance.initData.userCanUseCoupon, instance.initData.freightCoupon, instance.initData.userCanUseWhenNotSelectCoupon,instance.initData.fiveEnter)
    } else {
      instance.getedCoupon = options.getedCoupon;
      instance.couponProducts = options.couponProducts;
      instance.activityProducts = options.activityProducts;
    }
  ...
  initCouponList: function (userCanUseCoupon, freightCoupon = [], userCanUseWhenNotSelectCoupon = [],type) {
    if (userCanUseWhenNotSelectCoupon) {
      for (let i = 0; i < userCanUseWhenNotSelectCoupon.length; i++) {
        ...
        if(type){
          userCanUseWhenNotSelectCoupon[i].disable = false;
        }else{
