...
<template>
  <div class="list">
    ...
    <nav-top :noTitle="true" @backEvent="backEvent" v-if="componentName != 'AXJFShop'">
      ...
      <div class="right-btn" style="right: 0px; height: 44px; top: 0px; padding-right: 10px;" @click="listStyleEvent"
           v-if="pageType == ''">
        <i class="iconfont mall-datubiao theme_font_common" style="font-size: 24px;" v-if="listStyle == 2"></i>
        <i class="iconfont mall-qiehuan theme_font_common" style="font-size: 24px;" v-if="listStyle == 1"></i>
      </div>
    </nav-top>
    <nav-content>
      <div class="all-content">
        <div class="row axjf-tip line_bottom" v-if="componentName == 'AXJFShop'" @click="axPointsDetail">
...
<!--        </div>-->
        ...
        <div class="row row-filter top-showdow">
          <div class="filter-btn theme_font_black" @click="categoryEvent" v-if="categoryIdList == '' && dataType == ''">
          ...
          <div class="filter-btn theme_font_black" @click="filterEvent(3)"
               v-if="$store.state.globalConfig.pro_sku_view == '1'">
            <span class="single-line" :class="{theme_standard_font_i: filterType == 5}">人气</span>
          </div>
        ...
        <div class="list-content">
          ...
          <div class="no-list" v-if="isNoList && pageType == ''">
            ...
            <div class="tip theme_font_gray" v-if="selectedType == 1 && pageType == 'coupon'">当前门店无可用商品，请切换自提点查看可用商品</div>
            <div class="tip theme_font_gray" v-else-if="selectedType == 1">抱歉没有找到相应商品，请切换自提点试试吧</div>
            <div class="tip theme_font_gray" v-else>抱歉没有找到相应商品~</div>
            <Recommend v-if="selectedType == '2'"></Recommend>
            <PickupRecommend v-if="selectedType == '1' && searchKey != ''" :searchKey="searchKey"></PickupRecommend>
          </div>
        </div>
        <div class="activity-btns" v-if="pageType != '' && pageType != 'activity_nocart' ">
        ...
        <div
          ...
          <div v-else>
            <p
              ...
              v-if="$store.state.mall2.cartNum > 0">{{$store.state.mall2.cartNum}}</p>
    ...
    <div class="guide-div" v-if="showGuide">
      <img class="guide-img-1" @click="showGuide = false" src="static/image/mall2/guides/list-guide-1.png">
      <img class="guide-img-2" :style="{top: ($store.state.barHeight+$market.getNavHeight())+ 'px'}"
           src="static/image/mall2/guides/list-guide-2.png">
      <img class="guide-img-3" :style="{top: ($store.state.barHeight+$market.getNavHeight())+ 'px'}"
    ...
    <Filtrate :cateText="cateText" @filtrateEvent="filtrateEvent" @closeEvent="filtrateCloseEvent" :category="category" :cateLevel="cateLevel" v-if="showCreated"
...
<script>
  ...
  export default {
    ...
    created() {
      ...
      if (this.$store.state.list_guide_show == true && (this.$store.state.webtype == '0' || this.$store.state.webtype == '1' || this.$store.state.webtype == '2' || this.$store.state.webtype == '3')) {
      ...
      if (this.searchKey == '') {
        this.searchKey = this.$route.query.question ? this.$route.query.question : ''
      }
