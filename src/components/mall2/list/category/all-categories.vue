...
<template>
  <div class="all-categories" @click.stop="cancelSelect">
    <div class="categories-bottom" @click.stop="stop">
      <div class="pg-title" :style="{ marginTop: barHeight+'px' }">分类</div>
...
<script>
  export default {
    ...
    methods: {
      ...
      loadProCateList: function (storeOuCode) {
        let url = '/app/json/product/getAppProCategoryList';
        let params = {}
        if (storeOuCode) {
          params.storeOuCode = storeOuCode
          params.categoryListType = '1'
        }
