...
<template>
  <div class="list">
      ...
      <div class="all-content">
        <div class="row">
          <div class="local-div single-line" @click="selectAddressClick()">
            ...
            <span class="theme_font_gray single-line">{{location.area}}</span>
          </div>
          <div class="search-div theme_bg_white_f5"
            ...
            <span class="theme_font_tint single-line"
                  style="font-size: 14px;margin-left: 10px;">{{searchKey ? searchKey : '搜商品'}}</span>
          </div>
        </div>
    ...
    <AllCategory :barHeight="$store.state.barHeight" v-show="showAllCategory" @closeEvent="showAllCategory = false"
  ...
  export default {
    ...
    methods: {
      ...
      _loadProList: function (loaded) {
        ...
        this.$Loading.open();
        let url = '/app/json/product/getAppProListByDelivery';
        let paramsData = {
        ...
        paramsData.lng = this.location.posx
        paramsData.lat = this.location.posy

        if (this.businessType == '1' && this.location.selectedStore) {
          paramsData.storeOuCode = this.location.selectedStore.storeCode
        }
            ...
            if (data.status == 0) {
              ...
              if (this.nowPage != 0) {
                ...
                if (this.listData.length == 0) {
                  ...
                  if (this.selectedStore.storeCode == '') {
                    this.selectedStore = {
                      storeCode: this.listData[0].storeOuCode,
                      storeName: this.listData[0].storeName
                    }
                  }

                  this.sessionCartsEvent(this.selectedStore.storeCode)
      ...
      productEvent: function (product) {
        // console.log(product)
        let path = '/mall2/store-delivery/detail'
        this.$router.push({
          path: path,
          query: {
            id: product.id,
      ...
      addToCart: function (item) {
        ...
        this._settleCart(this.selectedStore.storeCode)
      },
      ...
      selectAddressClick: function () {//选择地址
        ...
        this.backRefresh = true;
        let path = '/mall2/store-delivery/address-list'
        let routeData = {
          path: path
        }
        if (this.businessType == 1) {
          routeData.query = this.selectedStore
          routeData.query.businessType = this.businessType
        }
        if (this.$userCenter.checkLogined(routeData, 0) == false) {
          return;
        }
    ...
    created() {
      ...
      if (location) {
        ...
        if (this.location.selectedStore) {
          this.selectedStore = this.location.selectedStore
        }
        this.nowPage = 0;
        this._loadProList()
      } else {
        this.$bridgefunc.getLocation((locationinfo) => {
          ...
          this.$mallCommon.nearAddress((address) => {
            ...
            localStorage.setItem('Address_List_Seleted', JSON.stringify(this.location))
            this._loadProList()
          }, locationinfo);
        })
      }
