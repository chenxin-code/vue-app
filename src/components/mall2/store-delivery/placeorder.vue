        ...
        <div class="scroll-div">
          <div
            ...
            <div
              ...
              <div class="flex-row">
                <div class="theme_font_black font-small">{{location.receiverName}}</div>
                <div class="theme_font_black font-small" style="margin-left: 10px;">{{$util.dealPhone(location.mobile)}}</div>
              </div>
            </div>
            <div
              v-else
              @click="toSelectAddress">
              <div class="flex-row address-area">请选择地址信息</div>
            </div>
          ...
          <div class="block-div" v-if="cardList.length != 0 && paramsData.orderCategory != 1 && limitType == 'true'">
            ...
            <div class="flex-row" @click="showYJChange = true">
              ...
              <div class="full theme_font_black right-margin" v-if="radio != 'a'" style="text-align: right;">
                <p v-if="$route.params.cardName">{{cardDetails.cardName ? cardDetails.cardName :
                  $route.params.cardName}}</p>
                <p v-else>选择可用易捷卡</p>
                <p v-if="$route.params.cardName" class="theme_font_tint font-small" style="padding-top: 3px;">
          ...
          <div class="block-div" v-if="recommendDisabled && (!cardType || (cardType && cardType == 551))">
            <div class="flex-row">
...
<script>
  ...
  export default {
    ...
    created() {
      ...
      // 填写默认推荐人(相对时间内)
      let referrerInvalidDate = this.$store.state.mall2.behalf
        .referrerInvalidDate;
      if (
        referrerInvalidDate &&
    ...
    methods: {
      ...
      toSelectAddress: function () {
        this.$router.push({
          ...
          query: {
            type: '2',
            storeOuCode: this.paramsData.storeOuCode
          }
        });
      },
      // 获取前面已填写的发票
      ...
      reloadSelectedCoupon: function () {
        ...
        for (let i = 0; i < this.occurData.userCanUseCoupon.length; i++) {
          ...
          if (this.occurData.userCanUseCoupon[i].isDeductFreight == 1) {
            arr2.push(this.occurData.userCanUseCoupon[i])
          } else {
            arr1.push(this.occurData.userCanUseCoupon[i])
          }
        }
        Coupon.reloadSelectedCoupon(arr1, arr2)
      },
      ...
      _reSettleCart: function (carts, cardNo) {
        ...
        if (this.location.id > 0) {
          paramsData.userAddressId = this.location.id
        }
        // paramsData.pickupId = '196'
        ...
        this.$http.post(url, paramsData).then(
          res => {
            ...
            this.complete = true
            if (data.status == 0) {
              sessionStorage.setItem('MDPS_Carts', JSON.stringify(carts))
      ...
      _submitOrder: function (carts) {
        ...
        if (this.location.id > 0) {
          paramsData.userAddress = this.location
            paramsData.userAddressId = this.location.id
        }
        ...
        this.$http.post(url, paramsData).then(
          res => {
            ...
            let data = res.data;
            if (data.status == 0) {
              sessionStorage.removeItem('MDPS_Carts')
    ...
    beforeRouteLeave(to, from, next) {
      if (to.path != "/mall2/store-delivery/address-list" &&
