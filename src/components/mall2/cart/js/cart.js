...
let cart = {
    ...
    /*购物车底部全选 icon类名*/
    getCheckedClass(arr, isEditing) {
        return this.isCheckedAll(arr, isEditing) ? 'mall-xuanzhong theme_standard_font' : 'mall-weixuanzhong theme_font_gray'
    },
    /*是否全部选中*/
    isCheckedAll(arr, isEditing) {
        let type = isEditing ? 'dchecked' : 'checked';
        return !(arr.find(item => !item[type]));
    },
    ...
    setOccSel: function(occuritem, isEditing, checkedStatus) {
        let checkKey = this.getCheckKey(isEditing);
        if (checkedStatus || checkedStatus >= 0) {
            occuritem[checkKey] = Boolean(checkedStatus);
        } else {
    ...
    /**
     * @description: 获取购物车选中商品数量
     * @param {*} occurArr   
     * @param {*} isEditing  true：编辑  | false 结算
     */
    getPayNum: function(occurArr, isEditing) {
        if (isEditing) {
            return 0;
        } else {
            let num = 0;
            occurArr.map(occuritem => {
                let checkKey = this.getCheckKey(isEditing);
                console.log(occurArr, checkKey, 'occurArr-----------')
                occuritem.store.map(storeitem => {
                    ...
                    num = storeitem.noActivityCart.reduce((acc, cur) => {
                        if (cur[checkKey]) {
                            acc += cur.number;
                        }
                        return acc
    ...
    // 获取所有商户下的所有项目
    getStoreSelOccur: function(occurArr, isEditing, type, checked) {
        let selArr = [];
        occurArr.map(occuritem => {
            occuritem.store.map(storeitem => {
        ...
        if (type == 'update') {
            for (let i = 0; i < selArr.length; i++) {
                let item = selArr[i];
                item.checked = checked >= 0 ? checked : occuritem.checked ? 0 : 1;
            }
        }
        return selArr;
    },
    ///获取收单机构下 所有的项目
    getSelOccur: function(occuritem, isEditing, type, checked) {
        let selArr = [];
        for (let j = 0; j < occuritem.store.length; j++) {
            let storeitem = occuritem.store[j];
            let arr = this.getSelStore(storeitem, isEditing, type);
