/**Created by liaoyingchao on 2020-12-08.*/

<template>
  <div class="presale-cart">
    ...
    <nav-content>
      <div class="row"
           v-if="presaleDeliverType != '1' && presaleDeliverType != '2'">
      ...
      <div class="list-content"
           :class="{notop: presaleDeliverType == '1' || presaleDeliverType == '2'}">
        <CartList v-for="(item,index) in topArr" :ref="getCartRef(item.type)" v-if="item.hasShow" v-show="deliverType == item.type" :deliverType="item.type" :isPresale="true" :key="index" :isEditing="isEditing" :orderCategory = "orderCategory" :vipUnitUserCode="vipUnitUserCode" @updateEdit="setEdit">
        </CartList>
      </div>
    </nav-content>
  </div>
...
<script>
  ...
  export default {
            ...
            if (address) {
              ...
              this.$nextTick(() => {
                if (this.$refs[comref]) {
                  ...
                  console.log('this.$store.state.mall2.zitiAddress', this.$store.state.mall2.zitiAddress)
                  console.log('this.$route.query.pickupAddress', this.$route.query.pickupAddress)
                  arr[0].getDataList();
    ...
    beforeRouteLeave(to, from, next) {
      if (to.path == this.lastPath || this.lastPath == '/' || this.lastPath == '') {
        this.$keepaliveHelper.deleteCache(this)
      }
      next()
    },
