...
<template>
  <div class="cart-list">
    <div class="cart-content" v-show="occurArr.length || invalidCart.length" ref="scrollwarp">
      <div class="scroll-content-div">
        <div :ref="getCartRef(index)" v-for="(occuritem,index) in occurArr" class="occur-div theme_bg_white">
            ...
            <div class="store-content" v-for="(activityitem,index) in storeitem.activity">
              ...
              <div class="activity-content">
                <CartItem v-for="(cartitem,index) in activityitem.cart" :isEditing="isEditing" :cartitem="cartitem"
                          ...
                          :key="cartitem.skuId" :orderCategory="orderCategory" :vipUnitUserCode="vipUnitUserCode"
                          @updateCart="updateCart" @deleteCart="deleteCart">
                </CartItem>
              </div>
            </div>
            <div class="store-content marT10">
              <div class="activity-content">
                <CartItem v-for="(cartitem,index) in storeitem.noActivityCart" :isEditing="isEditing"
                          :cartitem="cartitem"
                          :occuritem="occuritem" :storeitem="storeitem" @setCartSel="setCartSel" @numChange="numChange"
                          :key="cartitem.skuId" :orderCategory="orderCategory" :vipUnitUserCode="vipUnitUserCode"
                          @updateCart="updateCart" @deleteCart="deleteCart">
          ...
          <div class="pay-div-height">
            <PayDiv :occuritem="occuritem" :isEditing="isEditing" @setOccSel="setOccSel" @toPay="toPay"
                    @toDelete="toDelete" v-show="showFirstBt">
            </PayDiv>
          </div>
        </div>
      ...
      <!--固定在顶部的div-->
      <div v-for="(occuritem,index) in occurArr" class="occur-title theme_font_white theme_bg_ml_red occur-top-fixed"
           @click="setOccSel(occuritem)" v-show="fixedIndex==index">
        <i class="iconfont  font-large-x"
           :class="cartJS.getOccClass(isEditing,occuritem.checked,occuritem.dchecked)"></i>
        <span class="theme_font_black"> <i class="iconfont mall-shoukuanfang"></i> {{occuritem.ouName}}</span>
      </div>
      <!--固定在底部的div-->
      <PayDiv v-for="(occuritem,index) in occurArr" :occuritem="occuritem" class="pay-div occur-bt-fixed theme_bg_white"
              :isEditing="isEditing" @setOccSel="setOccSel" @toPay="toPay" @toDelete="toDelete"
              v-show="fixedIndex==index" :key="index">
      </PayDiv>
    </div>
    <div class="nodata-content" v-if="!(occurArr.length || invalidCart.length) && isHasRequest">
      ...
      <div class="recommend-div" v-if="$store.state.globalConfig.app_home_special_flag != 'cnooc' && this.isPresale != true">
        <Recommend :deliverType="this.deliverType" @addCartFinish="getDataList"></Recommend>
      </div>
    </div>
  </div>
</template>
