      ...
      <div class="flex-div">
        <div class="scroll-div">
          <div
            ...
            v-if="buyType != 'songli' && (deliveryType == 2 && occurData.needAddress == 1)&&$route.params.productType!=514|| deliveryType == 1 && $route.query.entryType != 'wish'&&$route.params.productType!=514">
            <div
            ...
            <div
              v-if="(!($store.state.mall2.selectAddress.id > 0)) && deliveryType == 2&&$route.params.productType!=514"
              @click="toSelectAddress">
              <div class="flex-row">请选择地址信息</div>
            </div>
            <div
              v-if="$store.state.mall2.zitiAddress.id > 0 && deliveryType == 1&&$route.params.productType!=514"
          ...
          <div class="block-div" v-if="deliveryType == 1&&$route.params.productType!=514">
            <div class="flex-row">
              <div class="title theme_font_black">提货人信息</div>
            </div>
            <div class="flex-row flex-row-smallbottom">
              <div class="label-middle theme_font_common" style="width: 2.6rem">姓名</div>
              <div class="full">
                <input
                  class="row-input"
            ...
            <div class="flex-row">
              <div class="label-middle theme_font_common" style="width: 2.6rem">联系方式</div>
              <div class="full">
            ...
            <div class="flex-row" @click="openPicker"
              ...
              <!--没有生鲜的时候才能选时间-->
              <div class="label-middle theme_font_common" style="width: 2.6rem">预约自提时间</div>
              <div
            ...
            <div class="flex-row" v-if="hasShengxian == true">
              ...
              <div class="pick-up-tip">
                提货时间：{{getSelectReceiveTime()}}到{{getReceiveEndTime()}}，超过自提时间后系统将自动取消订单<span v-if="$store.state.globalConfig.order_fresh_pickup_cancel_type != '0'">并退款</span><span v-if="$store.state.globalConfig.order_pickup_cancel_tip_extend">{{$store.state.globalConfig.order_pickup_cancel_tip_extend}}</span>
              </div>
            </div>
            <div class="flex-row" v-else-if="$store.state.globalConfig.order_pickup_cancel_tip_show != 'false' && $store.state.globalConfig.presaleType != 2">
              <div class="pick-up-tip">请于{{pickUpEndTime}}之前提货，超过自提时间后系统将自动取消订单并退款<span v-if="$store.state.globalConfig.order_pickup_cancel_tip_extend">{{$store.state.globalConfig.order_pickup_cancel_tip_extend}}</span></div>
            </div>
          </div>
            ...
            <div v-if="store.storeProData.num == 1">
              <div class="pro-row" v-for="prodata in store.storeProData.proArr">
                ...
                <div class="info-div">
                  ...
                  <div v-if="prodata.currentSelfActivity && prodata.currentSelfActivity.mktActivityType != ''" class="activity-type theme_bg_red">{{prodata.currentSelfActivity.mktActivityType}}</div>
                </div>
              </div>
              <div v-if="occurData.needPickupId == 1" @click="toSelectZitiAddress">
                <div class="pickup-shop">
                  <div style="flex: 1;">
                    <div class="flex-row">
  ...
  export default {
    ...
    computed: {
      serviceDateTime() {
        ...
        return this.selectedServiceTime.date.label + ' ' + this.selectedServiceTime.time.label
      },
      recommendDisabled() {
        return (
      ...
      recommend() {
        ...
        if (this.$store.state.globalConfig.rfrCodeType && this.$store.state.rfrCodeMap) {
          ...
          recommend = this.$store.state.rfrCodeMap[this.$store.state.globalConfig.rfrCodeType]
        }
        // URL入参，暂时定优先级最高，不清楚意义
        if (this.$route.params.recommendPhone || this.$route.query.recommendPhone) {
          recommend = this.$route.params.recommendPhone || this.$route.query.recommendPhone
        }
      ...
      canEditReferrer() {
        return this.$store.state.globalConfig.appCanEditReferrer == 1 || !this.recommend
      }
    },
    created() {
      console.log(this.$route.params.cutPrice)
      console.log(this.$store.state.globalConfig.order_pickup_cancel_tip_show != 'false')
    ...
    methods: {
      ...
      _submitOrder: function (carts) {
        ...
        if (this.productType == 8) {
          if (this.selectedServiceTime != '') {
            paramsData.selectReceiveTime = this.selectedServiceTime.date.dateStr + ' ' + this.selectedServiceTime.time.label + ':00'
          }
        }
        // 下单场景
          ...
          res => {
            ...
            if (data.status == 0) {
              if (paramsData.bargainOrderId) {
                this.updateBargainPayInfo({id: paramsData.bargainOrderId,orderId: data.data.orderId})
              }
