<template>
  <div class="coupon-list">
    <nav-top title="洗车站点" @backEvent="backEvent">
    </nav-top>
        ...
        <ul class="stores-c">
          <van-list
            ...
            <li
              ...
              <div class="top">
                <div class="left">
                  <img src="./scan-get-coupon/image/main.png" alt="" class="prod-img">
                </div>
                <div class="center">
                  <p class="prod-name">{{ item.ouName }}</p>
<!--                  <p class="business-hours">时间：{{`${item.openingTime.slice(0,5)}-${item.closingTime.slice(0,5)}`}}</p>-->
                  <p class="prod-address"><van-icon name="location-o" />{{item.addressFull}}</p>
                  <!--              <p class="prod-address">北京市大兴区大兴机场T1航站楼</p>-->
                </div>
              ...
              <div class="bottom">
                ...
                <div class="provide"><span class="icon"></span>由闪洗客提供</div>
                <!--              <van-button round plain color="#F02C2D" type="info" @click="storeDetail(item)">详情</van-button>-->
                <van-button round plain  style='background: #F02C2D;color:white' type="info" @click="useBenefits(item)" class="use">前往洗车</van-button>
              </div>
            </li>
          </van-list>
        </ul>
