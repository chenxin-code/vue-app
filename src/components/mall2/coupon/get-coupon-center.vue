  ...
  <div>
    ...
    <nav-content class="coupon-center" v-if="pageInit">
      <ul class="menu-list">
        <horizontal-scroll :dataList='defaultCouActivity'>
          ...
          <li
            ...
            class="menu-item"
            :class="{theme_standard_font: index === couponTypeIndex}"
            v-for="(item, index) in defaultCouActivity"
            v-if="item.activityModelList.length > 0"
            :key="index">
            <span class="text">{{ item.activityTag }}
                <i v-show="index === couponTypeIndex" class="line theme_bg_red"></i>
            </span>
          </li>
        </horizontal-scroll>
      </ul>
      <div class="coupon-list" v-if="couActivity.length">
        <template v-for="(item, index1) in couActivity">
          <template v-if="item.special">
            <exchange-coupon
              @moveDetail="toDetail(item,item2)"
              class="coupon-item"
              ...
              <!--状态10为待领取-->
              <div class="operate">
                <div v-if="item2.state == 10" class="btn theme_bg_red" @click="collectingVoucher(item, item2)">立即领取
                </div>
            ...
            <exchange-coupon
              ...
              <template v-else-if="isEnd(item)">
                <div class="operate btn-disabled">
                  <circle-progress size="44px"
                                   v-if="calCent(item2.distRewardCnt, item2.limitRewardCnt) >= 100 || item2.state != -1"
                                   :value="calCent(item2.distRewardCnt, item2.limitRewardCnt)">
              ...
              <template v-else>
                ...
                >
                  ...
                  <div v-if="calCent(item2.distRewardCnt, item2.limitRewardCnt) >= 100" class="btn theme_bg_red">已抢光
                  </div>
                  <template v-else>
                    <template v-if="item2.state == -1">
                      <img class="received" src="@/assets/state/received.png" alt="">
                      <div class="btn theme_bg_red" @click="couponProducts(item2)">去使用</div>
                    </template>
      ...
      <div class="no-coupon-wrapper" v-else>
        ...
        <p class="title">
          <router-link class="theme_standard_font" to="/coupon_market" v-if="!isHideExchangeCoupon">兑换电子券</router-link>
        </p>
      </div>
      <div class="my-coupon-link">
        <div class="link theme_standard_bg" @click="toDownloadShare" v-if="$store.state.globalConfig.couponCenterToDownloadShare == '1'">下载云南石油APP</div>
        <div class="link theme_standard_bg" @click="toCouponList" v-else>我的优惠券</div>
      </div>
    </nav-content>
  </div>
</template>
...
<script type="text/ecmascript-6">
  ...
  export default {
    ...
    mounted() {
      ...
      if (this.$store.state.globalConfig.hideCouponExchangeBtn) {
        this.isHideExchangeCoupon = true
      }
    },
    methods: {
      toDownloadShare: function () {
        this.$router.push({
      ...
      toDetail(coupon,coupon2) {
        this.$router.push({
          path: '/get_coupon_detail',
          // query:{
          //   coupon: JSON.stringify(coupon),
          //   coupon2: JSON.stringify(coupon2)
          // }
          query:{
            couTypeCode:coupon2.couTypeCode,
      ...
      collectingVoucher(item, item2) {
        if (item.special) { // 专享券
          ...
          this.$Loading.open()
          let params = {
            mktActivityId: item.mktActivityId,  // 活动ID
            ruleId: item.ruleId,
            // province: this.$store.state.indexData.province,
            // city: this.$store.state.indexData.city
            provinceId: this.$store.state.indexData.provinceId || '',
