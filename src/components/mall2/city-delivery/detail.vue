  ...
  <div class="detail mall2">
    ...
    <div ref="scrollView" class="scroll-div theme_bg_white" :style="getScrollTop()" v-if="detailShow">
                ...
                <pull-to ref="pullTo1"
                  ...
                  <div>
                    ...
                    <div class="base-info theme_bg_white">
                      ...
                      <!-- 添加到心愿单 -->
                      <div class="add-wish-btn theme_standard_bg" @click="addToCart">
                       加入购物袋
                      </div>
                      <!-- 添加到心愿单end -->
                      ...
                      <div class="seconds-kill" v-if="detailData.mktActivityPriceLevel == 3 || (showActivityTime == '1' && detailData.mktActivityPriceLevel != '')">
                        <div class="left-div">
                          ...
                          <div class="sk-row" v-else>
                            <div class="price theme_font_white">
                              <PriceOrder :showLevelImg="false" :detailColor="'white'" :detailitem="detailData"></PriceOrder>
                            </div>
                          </div>
                          <div class="sk-row">
                            <div class="sk-icon">
                            ...
                            <div class="o-price theme_font_white" :class="{'line-through': $store.state.globalConfig.priceShowType == 2}" v-if="detailData.activityPrice < detailData.salePrice">
                      ...
                      <div class="price" v-else>
                        ...
                        <span class="price-small theme_font_tint left-m-8"
                              v-if="detailData.dpedData.linePrice && detailData.activityPrice < detailData.salePrice" :class="{'line-through': $store.state.globalConfig.priceShowType == 2}">
                    ...
                    <div class="block-div theme_bg_white" v-if="productType == 4">
                      ...
                      <div class="pro-row" v-for="pro in detailData.packageSkuList">
                        ...
                        <div class="info-div">
                          <div class="title pro-title-height double-line theme_font_black" v-html="getSkuNameStr(pro)"></div>
                    ...
                    <!--金豆商城强制参与-->
                    <div class="block-div theme_bg_white" v-if="singleActivities_2.length > 0 && curSingleActivity != null && orderCategory != '1'">
                      <div class="flex-row">
                        <p class="sl-tag line_circle theme_border_red_i theme_font_red">{{curSingleActivity.activityType}}</p>
                        <p class="sl-title single-line">{{curSingleActivity.activityTitle}} <span class="theme_standard_font">{{getActivityStr(curSingleActivity)}} ¥{{$util.toDecimal2(curSingleActivity.activityPrice)}}</span></p>
                    ...
                    <div class="block-div theme_bg_white" v-if="singleActivities_2.length > 0 && curSingleActivity == null && orderCategory != '1'">
                      <div class="flex-row" v-for="saitem in singleActivities_2">
                        <p class="sl-tag line_circle theme_standard_bdr_i theme_standard_font">{{saitem.activityType}}</p>
                        <p class="sl-title single-line">{{saitem.activityTitle}} <span class="theme_standard_font">{{getActivityPrice(saitem)}} + ¥{{$util.toDecimal2(saitem.activityPrice)}}</span></p>
                        <p class="good-salespro-right" @click="joinSingleActivity(saitem)">参加</p>
                      </div>
                    </div>
                    <!--金豆商城无领券-->
                    <div class="block-div theme_bg_white" v-if="(!cardType || (cardType && cardType == 551)) && orderCategory != '1'">
                      <div class="flex-row" @click="couponEvent">
                        <div class="label-middle theme_font_gray">领券</div>
                        <div class="font-small theme_font_tint" v-if="detailData.couponModels == '' && jfhqList == ''">暂无可用优惠券</div>
                        ...
                        <div class="act-item-full theme_standard_bg theme_font_white"
                             ...
                             v-for="(couponModel, cidx) in jfhqList">{{couponModel.couTypeTitle}}</div>
                        <div class="full"></div>
                        <div>
                      ...
                      <div class="flex-row" v-for="(aactivity, aidx) in detailData.activityList"
                        ...
                        <div class="act-item-empty line_circle theme_standard_font theme_standard_bdr single-line">{{aactivity.type}}</div>
                    ...
                    <div class="block-div theme_bg_white" v-if="detailData.recommendSkuList && detailData.recommendSkuList.length > 0 && (!cardType || (cardType && cardType == 551))">
                      <div class="flex-row">
            ...
            <transition name="transition2">
              <div class="main-swiper" style="position: absolute; top: 44px;" v-show="proView == 2">
                <div class="btns-div theme_bg_white">
                  ...
                  <div class="btn-item line_left theme_font_gray" :class="{theme_standard_font_i: introductionIndex == 2}"
                       @click="introductionSelected(2)">规格参数
                  </div>
                  <div class="btn-item line_left theme_font_gray" :class="{theme_standard_font_i: introductionIndex == 3}"
                ...
                <div class="detail-div detail-div-1">
                  <pull-to ref="pullTo2"
                    ...
                    <div class="detail-content detail-bottom">
                      ...
                      <div v-html="introduction1" class="introduction-div" style="padding: 0" id="picMedia" v-if="introductionIndex == 1"></div>
      ...
      <div class="settle-div">
        <div class="cart-icon" @click.stop="showCartProducts = true">
          <i class="iconfont mall-gouwuche btn-icon theme_font_red"></i>
          <div class="num theme_bg_red" v-if="num>0">{{num}}</div>
        </div>
        <div class="info">
          <div class="price">
            <span>¥</span>
            <span class="big">{{occurData.payAmount || 0.0}}</span>
          </div>
          <div class="yf">
            另需配送费{{occurData.freightAmount || 0.0}}元
          </div>
        </div>
        <div class="btn theme_standard_bg" @click.stop="toPlaceOrder">去结算</div>
      </div>
    </div>
    <pop-view v-if="showPop" style="z-index: 200;" @closeEvent="popClose">
      <div class="pop-body">
        <div class="pro-row">
          <div class="img-div">
        ...
        <div class="bottom-btn theme_font_white theme_bg_red" @click="popSureEvent"
             v-if="popFlag == 'addToCart' && stockNum != 0 && canSale == true && detailData.status != '0'">加入购物袋
        </div>
    ...
    <pop-view v-if="showCartProducts" @closeEvent="showCartProducts = false">
      <div class="pop-body">
        <div class="pop-title theme_font_common">已选商品</div>
        <div class="pop-scroll-div">
          <div class="pro-row" v-for="(prodata, idx) in cartProducts">
            <div class="img-div">
            ...
            <div class="info-div">
              ...
              <div class="price pop-mdps">
                <PriceOrder :productData="prodata"></PriceOrder>
                <van-stepper
                  v-model="prodata.number"
                  class="wish-prod-stepper"
                  :max="prodata.saleNumMax"
                  :min="prodata.saleNumBegin"
                  :step="prodata.saleNumStep"
                  @change="onChange(prodata.skuId, prodata.number)"
                  integer/>
              </div>
            </div>
          </div>
  ...
  export default {
    ...
    data() {
      ...
      return {
        ...
        location: {},
        carts: [],
        resData_toPlaceOrder: {},
        paramsData_toPlaceOrder: {},
        cartProducts: [],
    ...
    methods: {
      onChange: function (skuId, number) {
        for (let i = 0; i < this.carts.length; i++) {
          if (this.carts[i].skuId == skuId) {
            this.carts[i].number = number
          }
        }
        if (this.carts.length) {
          this._settleCart(this.carts[0].storeOuCode)
        }
      },
      getCartsProducts: function () {
        let arr = []
        this.num = 0
        let arrCarts = []
        let store = this.occurData.store[0]
        for (let i = 0; i < store.activity.length; i++) {
          let carts = store.activity[i].cart
          for (let j = 0; j < carts.length; j++) {
            arr.push(carts[j])
            this.num += carts[j].number
            arrCarts.push({
              skuId: carts[j].skuId,
              storeOuCode: carts[j].storeOuCode,
              activityId: this.activityId,
              selfActivityId: carts[j].activityId,
              deliveryType: '2',
              number: carts[j].number
            })
          }
        }
        let ncarts = store.noActivityCart
        for (let j = 0; j < ncarts.length; j++) {
          arr.push(ncarts[j])
          this.num += ncarts[j].number
          arrCarts.push({
            skuId: ncarts[j].skuId,
            storeOuCode: ncarts[j].storeOuCode,
            activityId: this.activityId,
            selfActivityId: ncarts[j].activityId,
            deliveryType: '2',
            number: ncarts[j].number
          })
        }
      ...
      saveTCPS_Carts(){
        sessionStorage.setItem('TCPS_Carts', JSON.stringify({
          carts: this.carts,
      ...
      getTCPS_Carts(){
        return JSON.parse(sessionStorage.getItem('TCPS_Carts'));
      },
      isPayTime: function () {
      ...
      popSureEvent: function () {
        if (this.popFlag == 'addToCart') {
          this._addToCart(this.detailData)
      ...
      _addToCart: function (item) {
        ...
        let has = false
        for (let i = 0; i < this.carts.length; i++) {
          if (this.carts[i].skuId == item.skuId) {
            has = true
            if (this.stockNum < parseInt(this.carts[i].number) + 1) {
              this.$Toast('当前商品库存不足！')
              return;
            }
            this.carts[i].number = parseInt(this.carts[i].number) + this.selectedNum
          }
        }
        if (has == false) {
          if (this.stockNum < 1) {
        ...
        this._settleCart(item.storeOuCode)
      },
      _settleCart: function (storeOuCode) {
        this.num = 0
        for (let i = 0; i < this.carts.length; i++) {
          this.num += parseInt(this.carts[i].number)
        }
        ...
        this.$http.post(url, paramsData).then(
          res => {
            ...
            let data = res.data;
            if (data.status == 0) {
              this.paramsData_toPlaceOrder = paramsData
              this.resData_toPlaceOrder = data.data
      ...
      toPlaceOrder: function () {
        ...
        if (this.resData_toPlaceOrder.occur.length > 0) {
          let occurData = this.resData_toPlaceOrder.occur[0]
          if (occurData.startAmount > 0 && occurData.startAmount > occurData.realAmount) {
            this.$Toast(occurData.startAmount + '元起送！')
            return ;
          }
        }
        let params = {
          res: this.resData_toPlaceOrder,
          paramsData: this.paramsData_toPlaceOrder,
          deliveryType: '2'
        };
    ...
    created() {
      ...
      let carts = this.getTCPS_Carts()
      if (carts && carts.cityId == this.location.cityId) {
