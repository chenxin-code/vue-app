...
<template>
  <div class="placeorder mall2">
    <nav-top @backEvent="$router.go(-1)"  title="同城配送"></nav-top>
  ...
  export default {
    ...
    created() {
      ...
      if (location) {
        ...
        if (this.$store.state.mall2.selectAddress.id > 0 && this.$store.state.mall2.selectAddress.cityId != this.location.cityId) {
          this.$store.state.mall2.selectAddress = this.location
          this.$MessageBox.confirm("同城配送只支持同城地址，请选择收货地址", "提示", {
      ...
      _submitOrder: function (carts) {
        ...
        this.$http.post(url, paramsData).then(
          res => {
            ...
            let data = res.data;
            if (data.status == 0) {
              sessionStorage.removeItem("TCPS_Carts");
