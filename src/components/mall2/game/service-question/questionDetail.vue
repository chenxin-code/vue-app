<template>
  <div id="body">
    <nav-top @backEvent="$router.go(-1)" title="加油评价" v-if="!firmdetailSubject">
        ...
        <div class="top" v-for="(item, index) in answerList" :key="index">
          <div class="singleList" v-if="item.kindId === 1">
            <div class="divObj">
              ...
              <!--<span>{{index+1}}、</span><span v-html="listItem.title"></span>-->
              <div class="titleHead">
                <span v-html="listItem.title"></span>
              </div>
          ...
          <div class="singleList multipleList" v-if="item.kindId === 2">
            <div class="divObj">
              <!--<span>{{listItem.orderNo}}、</span><span v-html="listItem.title"></span>-->
              <div class="titleHead">
                <span v-html="listItem.title"></span>
              </div>
...
export default {
  ...
  mounted() {
    if(this.firmdetailSubject == '加油凭证') {
      ...
      this.questionUrlData(params).then(res => {
        console.log(res);
        if (res.data.result === "success") {
          this.firmId = res.data.data.id
          this.loadData();
        }
      });
    } else {
  ...
  methods: {
    loadData() {
      ...
      if (this.firmdetailSubject == '加油凭证') {
        params1.orderId = that.orderIds
        params1.id = that.firmId
      } else {
        params1.orderId = that.paramsData.orderId
        params1.id = that.paramsData.id
      }
    ...
    sharegoods: function() {
      ...
      let shareData = {
        ...
        detailurl: `${this.$store.state.globalConfig.wxBaseUrl}${Config.shareUrl}/service/questionDetail?shareUserid=${this.$store.state.userInfo.userId}`
      };
