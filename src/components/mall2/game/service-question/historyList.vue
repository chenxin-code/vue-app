<template>
  <div class="body">
    <nav-top title="历史评价" @backEvent="$router.go(-1)"></nav-top>
    <nav-content>
        ...
        <div class="contain" v-if="answerList.length>0">
          <div v-for="(item,index) in answerList">
            <div class="publicInvoice" v-if="item.serviceType === 2">
              <p class="p"><span>加油评价</span><span>已评价</span></p>
              <van-cell-group>
                <van-field v-model="item.createTime" disabled label="加油时间"/>
                <van-field v-model="item.ouName" disabled label="油站名称"/>
              </van-cell-group>
              <div class="answerd" @click="answers(item)"><a class="theme_bg_red">评价详情</a></div>
            </div>
          </div>
        </div>
        <div class="contain" v-if="answerList.length==0">
          <span class="emptyState">暂无评价记录</span>
        </div>
      </div>
    </nav-content>
  </div>
