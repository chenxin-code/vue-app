<template>
  <div class="body">
    <nav-top title="问卷列表" @backEvent="$router.go(-1)">
      <div class="shareBox">
        <div>
        ...
        <div class="contain" v-if="answerList.length>0">
          <div v-for="(item,index) in answerList">
            <div class="publicInvoice" v-if="item.serviceType === 2">
              <p class="p"><span>{{item.title}}</span><span>未填写</span></p>
              <van-cell-group>
              ...
              <div class="answerd" @click="dati(item)"><a class="theme_bg_red">填写问卷</a></div>
            </div>
            <div class="publicInvoice" v-if="item.serviceType === 1||item.serviceType === 4">
              <p class="p"><span>{{item.title}}</span><span>未填写</span></p>
              <van-cell-group>
                <van-field v-model="item.questionStartTime" disabled label="开始时间"/>
                <van-field v-model="item.questionEndTime" disabled label="结束时间"/>
                <div class="publicInvoiceDiv" @click="detail(item.questionPrize)">
                  <van-field label="问卷奖品" disabled/>
                  <span class="img">
              <img src="static/image/invoice/arrows.png" alt>
      ...
      dati(index) {
        ...
        that.participateData(params).then(res => {
          ...
          if (res.data.result === 'success') {
            that.$router.push({path: '/subject', query: {
                subId: that.detailId,
                orderId: that.orderId
              }})
          } else {
