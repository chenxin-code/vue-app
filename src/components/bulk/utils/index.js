...
function getUrlQueryParams(url) {
  var w = (url || location.hash).indexOf("?");
  var hashQuery = (url || location.hash).substring(w + 1);
  var searchQuery = window.location.search.substring(1);
  var query = searchQuery || hashQuery;
  var vars = query.split("&");
  var obj = {};
...
export function getLocation(url) {
  ...
  const query = getUrlQueryParams(href);
  const findLen = href.indexOf("?");
  const _url = href.substring(0, findLen);
  console.log("query--->", query, _url);
  const urls = _url + toQuery(query);
