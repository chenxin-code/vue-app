<template>
  <div class="body">
    <nav-top @backEvent="$router.go(-1)" title="石化钱包加油订单"></nav-top>
    <nav-content>
        ...
        <div class="operation">
          <div class="divBox">
            ...
            <div class="operationImg">
              ...
              <div class="titleContent">
                <span class="titleMessage">{{item.ouName}}</span>
              </div>
            </div>
          </div>
          <div class="mall-jiantou">
            <span class="span">{{item.realAmount}}</span>元
          </div>
        </div>
      </div>
  ...
  export default {
    ...
    methods: {
      loadIndex() {
        ...
        that.walletData(params).then(res => {
          console.log(res.data.data);
          if (res.data.result) {
            that.listData = res.data.data || []
            // that.listData = [
            //   {completeTime: '2019-05-15 17:13', storeName: '加油卡加油开票 ', payAmount: 300.00,id: 1},
            //   {completeTime: '2019-05-15 17:13', storeName: 'APP自助加油开票 ', payAmount: 73.00,id: 2},
            //   {completeTime: '2019-05-15 17:13', storeName: '商城购物开票', payAmount: 192.00,id: 3}
            // ]
          }
        });
      },
      selectData() {
        ...
        that.radioList.forEach(function(item) {
          that.totalMoney = Number((that.totalMoney + Number(item.realAmount)).toFixed(2))
        })
      },
      allSelect() {
        let that = this
      ...
      time() {
        console.log(this.$store.state.globalConfig.gz_timeLimitInvoicing==1)
