<template>
  <div class="image-ad" :style="getFloatUp()" ref="box" v-if="pageRefresh && $market.moduleIsShow(moduleData)">
    <div class="f-div" v-if="!usePorpData"></div>
    <!-- 轮播广告 -->
    <div v-if="moduleData.type == 'lbt'" class="swiper-box lbt " :style="getSidesStyle()">
      <swiper :options="lbtOption" ref="mySwiper">
        <swiper-slide v-for="(nav, index) in moduleData.rows" :key="index" :class="hotareaClass(nav)">
          <img ref="swiperImg" class="swiper-img" :src="nav.imgUrl" :style="swiperSlideStyle" >
          <CornerMark v-if="nav.cornerMark" :itemData="nav" :moduleData="moduleData"></CornerMark>
        </swiper-slide>
        <div v-show="moduleData.rows.length != '1'" class="swiper-pagination " slot="pagination"></div>
      </swiper>
      <div class="notice-div" v-if="moduleData.notice && moduleData.notice != ''">
        <img class="notice-img" src="static/images/image-ad/notice.png" alt="">
        <p class="notice-title">通知公告:{{moduleData.notice}}</p>
      </div>
    </div>
    <!-- 功能导航 -->
    <div v-else-if="moduleData.type == 'gndh'" class="gndh swiper-box" :style="getSidesStyle(true)">
      <swiper :options="gndhOption" ref="mySwiper" :style="getBottomStyle()">
        <swiper-slide v-for="pageIdx in getPageLength" :key="pageIdx-1">
          <div class="gndh-page">
            <div class="gndh-item fill-bgp" :class="{'width-5' : moduleData.columnNum == '5', 'width-3' : moduleData.columnNum == '3', 'width-2' : moduleData.columnNum == '2', ...hotareaClass(nav, true)}"
                 @touchstart="gndhTouchStart"  @touchend="gndhTouchEnd"
                 v-for="(nav, index) in getPageItems(pageIdx-1) " :key="index"
            >
              <div v-if="moduleData.style == 'imgStyle'" @click.prevent="hotEvent(nav.idCode);enterNav(nav)">
                <CornerMark v-if="nav.cornerMark" :itemData="nav" :moduleData="moduleData"></CornerMark>
                <img ref="swiperImg" class="swiper-img" :src="nav.imgUrl" :style="swiperSlideStyle">
              </div>
              <div v-if="moduleData.style == 'imgTextStyle'" @click.prevent="hotEvent(nav.idCode);enterNav(nav)">
                <CornerMark v-if="nav.cornerMark" :itemData="nav" :moduleData="moduleData"></CornerMark>
                <img ref="swiperImg" class="swiper-img" :src="nav.imgUrl" :style="swiperSlideStyle">
                <p class="title" :style="{'background-color': moduleData.bgColor}"> {{nav.title}} </p>
              </div>
              <div v-if="moduleData.style == 'minImgTextStyle'" @click.prevent="hotEvent(nav.idCode);enterNav(nav)">
        ...
        <div v-show="gndhPageArr.length > moduleData.columnNum * moduleData.rowsNum" class="swiper-pagination"
             slot="pagination"></div>
      </swiper>
    </div>
    <!-- 一行一个 -->
    <div v-else-if="moduleData.type == 'yhyg'" class="swiper-box" :style="getSidesStyle()">
      <div ref="mySwiper">
        <div v-for="(nav, index) in moduleData.rows" :key="index" :class="hotareaClass(nav)">
          <img ref="swiperImg" class="swiper-img" :src="nav.imgUrl" :style="swiperSlideStyle" @click="hotEvent(nav.idCode);enterNav(nav)">
          <div v-if="index+1 !== moduleData.rows.length" :style="{'margin-bottom': moduleData.space+'px'}"></div>
        </div>
      </div>
    </div>
    <!-- 列表导航 -->
    <div v-else-if="moduleData.type == 'lbdh'" class="swiper-box" :style="getSidesStyle()">
      <swiper :options="lbdhOption" ref="mySwiper">
        <swiper-slide v-for="(nav, index) in moduleData.rows" :key="index">
          <div class="cells">
            <div :class="hotareaClass(nav)" class="cell lbdh-row fill-bgp" @click="hotEvent(nav.idCode);enterNav(nav)">
              <div class="cell__hd padding-left relative">
                <CornerMark v-if="nav.cornerMark" :itemData="nav" :moduleData="moduleData"></CornerMark>
                <img class="" ref="swiperImg" width="35px" height="35px" :src="nav.imgUrl">
              </div>
              <div class="cell__bd padding-left">{{nav.title}}</div>
              <div class="cell__ft padding-right"><img width="15px" height="15px" src="static/images/icon-more.png"
                                                       alt=""></div>
            </div>
          </div>
        </swiper-slide>
...
<script>
  ...
  export default {
    ...
    computed: {
      ...
      //获取swiper页数
      getPageLength() {
        return parseInt((this.gndhPageArr.length - 1) / (this.moduleData.columnNum * this.moduleData.rowsNum)) + 1;
      }
    },
    mixins: [hee],
    data() {
    ...
    created() {
      if (this.usePorpData == true) {
        if (this.moduleData.type == 'lbt' || this.moduleData.type == 'gndh' || this.moduleData.type == 'lbdh') {
    ...
    methods: {
      ...
      modeleReshow: function () {
        this.$bridgefunc.getItem(this.pageData.moduleList[this.myIndex].moduleId+'gndhPageArr', result => {
      ...
      getPageItems: function (pageIdx) {
        ...
        return this.gndhPageArr.slice(this.moduleData.columnNum * this.moduleData.rowsNum * pageIdx, this.moduleData.columnNum * this.moduleData.rowsNum * (pageIdx + 1));
      },
      ...
      resetSwiper: function (val) {
        ...
        if (layoutState == 'lbt') {
          ...
          this.swiperSlideStyle = "height: " + (this.clientWidth * (100 - sidesMargin - sidesMargin) / 100) * relativeSize + 'px'
        }
        if (layoutState == 'gndh') {
          ...
          this.swiperSlideStyle = "height: " + (this.clientWidth * (100 - sidesMargin - sidesMargin) / 100) / data.columnNum * relativeSize + 'px'
