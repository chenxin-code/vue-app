<template>
  <div class="top-nav" :class="{pfixed: usePorpData}">
    <div class="default-div default-div-bg" v-if="myData.type == 'default'" :style="getStyleHeaderBg(myData)">
      <div class="flex-box" :style="{'min-height':this.$market.getNavHeight()+'px'}">
        ...
        <div class="nav-top-title"
             :style="{'line-height':this.$market.getNavHeight()+'px', 'color': myData.titleColor}">{{myData.title}}
        </div>
      </div>
    </div>
    <div class="default-div" v-if="myData.type == 'custom'" :style="getStyleHeaderBg(myData)">
      <div class="bg-div" :style="getBgDivStyle(myData)"></div>
      <div class="user-msg" :class="{'deeper-text': myData.isFloat != true || scrollSite > 60}" :style="{top: barHeight + 'px'}" v-if="$store.state.login.token != '' && myData.isShowUserMessage">尊敬的{{$store.state.mall2.myAssets.levelNoStr}}会员，您的积分余额：{{$store.state.mall2.myAssets.surplusPoints}}</div>
      <div class="flex-div" :style="{'min-height':this.$market.getNavHeight(true)+'px',backgroundImage:`url(${myData.bgImgUrl2})`}" v-if="myData.bgStyle != 'white'">
        ...
        <div class="search-div other-full space-margin" :class="{'search-div-bg': myData.isFloat == true && scrollSite < 60, ...hotareaClass({idCode: myData.idCode + 'clickSearch'}, true)}" v-if="myData.middelControl == 'search'">
          <div class="search-bg" @click="hotEvent('clickSearch');$market.clickSearch()">
            <div style="height: 20px;line-height: 20px;" class="search-main">
              商品名称
            </div>
            <div class="search-icon">
              <img style="display: block;" width="16px" height="16px" :src="myData.isFloat == true && scrollSite < 60 ? 'static/images/header/search-w.png' : 'static/images/header/search.png'"/>
            </div>
          </div>
        </div>
        ...
        <div class="img-btn-pading space-margin" :class="{'img-btn-bg': myData.isFloat == true && scrollSite < 60, ...hotareaClass({idCode: myData.idCode + 'showScan'}, true)}" v-if="myData.isShowScan" @click="hotEvent('showScan');$market.showScan()">
          <img :style="imgStyle" src="static/images/header/scan-w.png"/>
        </div>
        <div class="img-btn-pading space-margin" :class="{'img-btn-bg': myData.isFloat == true && scrollSite < 60, ...hotareaClass({idCode: myData.idCode + 'showShare'}, true)}" v-if="myData.isShowShare" @click="hotEvent('showShare');showShare()">
          <img :style="imgStyle" src="static/images/header/share-w.png"/>
        </div>
        <div class="img-btn-pading space-margin" :class="{'img-btn-bg': myData.isFloat == true && scrollSite < 60, ...hotareaClass({idCode: myData.idCode + 'showQrcode'}, true)}" v-if="myData.isShowQrCode" @click="hotEvent('showQrcode');showQrcode()">
      ...
      <div class="flex-div white" :style="{'min-height':this.$market.getNavHeight(true)+'px',backgroundImage:`url(${myData.bgImgUrl2})`}" v-if="myData.bgStyle == 'white'">
        ...
        <div class="img-btn-pading space-margin" :class="{'img-btn-bg': myData.isFloat == true && scrollSite < 60, ...hotareaClass({idCode: myData.idCode + 'showScan'}, true)}" v-if="myData.isShowScan" @click="hotEvent('showScan');$market.showScan()">
          <img :style="imgStyle" :src="myData.isFloat == true && scrollSite < 60 ? 'static/images/header/scan-w.png' : 'static/images/header/scan.png'"/>
        </div>
        <div class="img-btn-pading space-margin" :class="{'img-btn-bg': myData.isFloat == true && scrollSite < 60, ...hotareaClass({idCode: myData.idCode + 'showShare'}, true)}" v-if="myData.isShowShare" @click="hotEvent('showShare');showShare()">
          <img :style="imgStyle" :src="myData.isFloat == true && scrollSite < 60 ? 'static/images/header/share-w.png' : 'static/images/header/share.png'"/>
        </div>
        <div class="img-btn-pading space-margin" :class="{'img-btn-bg': myData.isFloat == true && scrollSite < 60, ...hotareaClass({idCode: myData.idCode + 'showQrcode'}, true)}" v-if="myData.isShowQrCode" @click="hotEvent('showQrcode');showQrcode()">
          <img :style="imgStyle" :src="myData.isFloat == true && scrollSite < 60 ? 'static/images/header/qrcode-w.png' : 'static/images/header/qrcode.png'"/>
        </div>
        <div class="img-btn-pading space-margin" :class="{'img-btn-bg': myData.isFloat == true && scrollSite < 60, ...hotareaClass({idCode: myData.idCode + 'showService'}, true)}" v-if="myData.isShowService" @click="hotEvent('showService');$market.toYHTService()">
...
<script>
  ...
  export default {
    ...
    mounted() {
      ...
      if (this.$store.state.webtype == 3 && this.$store.state.isAddToFavorites != 1 && this.$store.state.globalConfig.hasFavoritesDescriptionPic) {
        this.wechatDescPicUrl = this.$store.state.globalConfig.hasFavoritesDescriptionPic || ''
  ...
  .top-nav {
    ...
    .add-wechat-favorites {
      ...
      .add-wechat-btn {
        ...
        font-size 11px;
        border-radius 11px;
        animation addWechatAni .7s linear infinite alternate;
      }
    }
