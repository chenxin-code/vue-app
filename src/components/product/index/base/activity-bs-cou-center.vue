...
<template>
  <Modal class="top-pop" v-model="show" title="大屏领券活动中心" @on-ok="sureEvent" :scrollable="true" width="800" @on-visible-change="changeProductsModal">
    <!--<Row>-->
  ...
  export default {
    ...
    methods: {
      ...
      _loadActivityList: function () {
        let url = '/app/json/app_market/queryScreenGetCouActivityAward';
        let paramsData = {
          token: this.$store.state.login.token
        };
    ...
    created() {
      ...
      Bus.$on("showBSCouActivityCenter", busData => {
