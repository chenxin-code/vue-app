        ...
        <div ref="preview_scroll" class="scroll-div" @scroll="pageSite" v-else>
          <van-pull-refresh v-model="refreshing" @refresh="topRefresh">
            <van-list
              ...
              @load="moduleLoadMore"
            >
              <component v-for="(item, index) in pageData.moduleList" v-if="moduleIsShow(item.data)" @componentEvent="componentEvent" :ref="'page_module_'+index" :is="item.code" :myIndex="index" :usePorpData="true" :isScrollPage="true" :propData="pageData" :scrollSite="scrollSite" :barHeight="barHeight" :key="index"></component>
