<template>
  <div class="product-list" :class="{'min-height' : usePorpData != true}" :style="$market.getModuleBgStyle(moduleData)" v-if="(getModuleIsShow() && moduleData.rows.length > 0) || usePorpData != true">
    <div class="title-img" v-if="moduleData.titleImg != ''">
      <img :src="moduleData.titleImg"/>
      <div class="title-content" v-if="moduleData.showTime && moduleData.type == '3' && moduleData.useApi == 3">
        <!--<div class="field">{{fieldString}}</div>-->
        <div class="remaining">{{timeText}}</div>
        <Countdown @timeOut="myTimeOut" :endTime="endTime" type="theme_bg_quan" v-if="showCountdown"></Countdown>
    ...
    <div class="channels-div" v-if="moduleData.useApi == '4' && moduleData.channels && moduleData.channels.length > 1">
      <div class="channel" :class="{selected: selectedChannel && selectedChannel.id == channel.id}" v-for="channel in moduleData.channels" @click="selectChannel(channel)">{{channel.name}}
      </div>
    </div>
    <ProductLlistGrid :myIndex="myIndex" :usePorpData="usePorpData" :propData="propData"
                      v-if="moduleData.type == '1'"></ProductLlistGrid>
    <!--<ProductLlistDetail :myIndex="myIndex" :usePorpData="usePorpData" :propData="propData" v-if="moduleData.type == '2'"></ProductLlistDetail>-->
    <ProductLlistHScroll :myIndex="myIndex" :usePorpData="usePorpData" :propData="propData"
                         v-if="moduleData.type == '3'"></ProductLlistHScroll>
  </div>
</template>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      getEndTime: function () {
        if (this.moduleData.showTime && this.moduleData.type == '3' && this.moduleData.useApi == 3 && this.moduleData.rows.length > 0) {
      ...
      _getCurrentDatas: function (skuIds, currentData) {
        ...
        if (this.$store.state.login.token && this.$store.state.login.token != '' && this.usePorpData == true) {
          paramsData.token = this.$store.state.login.token;
        }
        if (skuIds) {
          ...
          if (this.moduleData.useApi == 2) {
            ...
            if (this.moduleData.productFrom.multiple == '2' && backCates && backCates.length>1) {
              // 多选分类
              let categoryIdList = []
            ...
            paramsData.activityId = currentData.activityId ? currentData.activityId : currentData.id
          } else if (this.moduleData.useApi == 4) {
            if (!currentData) {
          ...
          res => {
            ...
            if (data.status == 0) {
              ...
              newRows.forEach(item=>{
                ...
                this.$set(item,'selfCornerMarkUrl',row ? row.selfCornerMarkUrl : '')
              })
