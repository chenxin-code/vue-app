...
<template>
  ...
  >
    ...
    <div class="mobile-preview">
      <div class="jiankangma" v-if="pageType == 'jiankangma'">
        <div class="back-div" @click="$router.go(-1)">
          <i
            class="iconfont mall-fanhui theme_font_white"
        ...
        <div class="get-coupon" @click="toGetCoupon">
          消费一码通顾客专享礼遇
        </div>
      </div>
      <div
      ...
      >
        <component
          ...
          @getPageData="getPageData(pgCode, -1)"
        ></component>
      </div>
      <div
        class="full-div"
    ...
    <HyActivityModal
      ref="hyActivity"
      v-if="$store.state.globalConfig.app_home_special_flag == 'cnooc'"
    ></HyActivityModal>
    <van-dialog
      ...
      show-cancel-button
    >
      <div style="padding: 40px 0; text-align: center">您有未支付加油订单</div>
    </van-dialog>
    <van-popup
    ...
    >
      ...
      >
      <div v-if="hasPrize">
        <p v-if="prize.length > 0 && prize[0].prizeType != '20030'">
          * 完成评价后可获赠{{ prize[0].prizeNum }}张{{ prize[0].prizeName
          }}{{ prize.length > 1 ? "等" : "" }}
        </p>
        <p v-if="prize.length > 0 && prize[0].prizeType == '20030'">
          * 完成评价后可获赠{{ prize[0].prizeName }}{{ prize[0].prizeNum }}次{{
            prize.length > 1 ? "等" : ""
          }}
        </p>
      </div>
...
export default {
  ...
  components: {
    ...
    RebateBonus: () =>
      import("@/components/usercenter/setting/rebate-bonus/index"),
    FlashPayment: () =>
      import("@/components/product/flashPayment/flashPayment"),
    ...
    PurchaseService: () =>
      import("@/deploy/chum/views/purchase-service/purchase-service.vue"),
    CnoocPayPersonal: () =>
      import("@/deploy/cnooc/views/pay-personal/pay-personal.vue"),
  ...
  methods: {
    ...
    tabbarEvent: function(index) {
      ...
      this.$actionStatistics.commonAction(
        "140003",
        "",
        "",
        "",
      ...
      this.$nextTick(() => {
        if (this.getComRef(index) && this.$refs[this.getComRef(index)]) {
          ...
          if (
            item.code == "PreviewComponent" &&
            this.$refs[this.getComRef(index)][0]
          ) {
      ...
      staticDataRequest.request(url, funcName, args).then(data => {
        ...
        if (data.status == 0) {
          ...
          if (this.$route.query.defaultCurrentTab) {
            this.$nextTick(() => {
              if (this.$refs.tabbar) {
                this.$refs.tabbar.tabbarEvent(
                  this.$route.query.defaultCurrentTab
                );
              }
            });
          }
    ...
    getSubPageDatas: function() {
      ...
      if (this.$store.state.indexData.useSaveIndex == true) {
        ...
        if (
          ...
          this.$store.state.indexData.selectedIndex > -1
        ) {
    ...
    getLocation: function() {
      this.$bridgefunc.getLocation((locationinfo, precision) => {
        ...
        if (
          this.$store.state.globalConfig.onlyProvince_area == "1" &&
          this.getShortName(locationinfo.province) !=
            this.getShortName(this.$store.state.globalConfig.province)
        ) {
          ...
          if (
            ...
            this.getShortName(this.$store.state.indexData.city) !=
            this.getShortName(locationinfo.city)
          ) {
            this.$messagebox
              .confirm(
                "离您最近的城市是" +
      ...
      this.$http
        ...
        .then(res => {
          ...
          if (data.status == 0) {
            ...
            this.$store.state.mall2.zitiAddress.storeName =
              data.data.storeName ||
              this.$store.state.refuelStation.stationName;
            this.$store.state.mall2.zitiAddress.mchOuCode =
              data.data.merchantOuCode;
            if (
