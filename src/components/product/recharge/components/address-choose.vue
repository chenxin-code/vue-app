      ...
      <div class="address-choose" v-show="isShowAddress" ref="addressChoose">
        <div class="nav-bar-wrapper">
          <div class="nav-bar">
            ...
            <van-icon class="icon-btn" name="close" @click="closeAddress"/>
          </div>
        </div>
        <scroll class="address-list" ref="scroll" :click="true" :data="addressList">
          <div class="address-item" v-for="(item, index) in addressList" :key="index">
            <van-icon size="26px" name="add-o" @click="selectAddress(item)" />
            <span class="text"  @click="changeArea(item)">{{item.provinceName}}</span>
          </div>
        </scroll>
      </div>
    </transition>
    ...
    methods: {
      ...
      selectAddress(item) {
        if (this.canClick) {
          ...
          this._getCityList(item.provinceId)
        }
      },
      // _getAddressList (parentId = -1) {
      //   this.$Loading.open()
      //   let url = '/app/json/fee_life_order/queryUtilityProvince'
      //   let params = {
