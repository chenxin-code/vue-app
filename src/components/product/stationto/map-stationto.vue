...
<template>
  <div class="mapbot">
    <nav-top @backEvent="backEvent" :noBack="!hasBack" title="油站导航"></nav-top>
    <nav-content>
      <template v-if="isOpenLocation === 1">
        ...
        <oil-station-list
          ...
          :tags="tags"
          :stations="stations"
          :stationType="$route.query.stationType"
          ref="stationList"
      ...
      <template v-if="isOpenLocation === 0">
        <div class="no-gps-container">
          <img class="img" src="./image/gps.png" alt="">
          <p class="msg">【请您开启GPS定位附近中国石化加油站】</p>
    ...
    <div
      ...
      <div class="searchmorebot">
        <div
          class="qrcode-top "
          :style="{height: (this.$store.state.barHeight + 44)+ 'px'}"
          @backEvent="searchBackEvent">
          <div
            class="navcontent"
            :style="{top: this.$store.state.barHeight+ 'px','height': 44 + 'px'}">
            ...
            <div class="search-div">
              ...
              <div class="form-control">
                <input v-model="searchText" class="input-none" type="search" @keyup.enter='searchEntery' placeholder="输入地点或商区名称"/>
              </div>
            </div>
            <div class="cancel" @click="cancelEvent">取消</div>
          </div>
        </div>
        <div class="history-wrapper" :style="{top: (this.$store.state.barHeight + 44)+ 'px'}">
          <div
...
<script>
  ...
  import stationToJS from './js/stationto'
  ...
  export default {
    ...
    data() {
      return {
        ...
        mapslogoTruck:'static/image/station/maplogo_truck.png',
      }
    },
    created() {
    ...
    methods: {
      ...
      addNewLocations: function () {
        ...
        for (let i = 0; i < this.stations.length; i++) {
          ...
          var myIcon = null;
          // 如果该油站有卡车翼家的标识，则添加卡车翼家图标
          if (tempSt.truck == 1) {
            myIcon = new BMap.Icon(this.mapslogoTruck, new BMap.Size(23, 30))
          } else if (tempSt.stationStatus && tempSt.stationStatus != '02') {
      ...
      requestStations: function (loaded) {
        ...
        let url = '/app/json/station/getStationMoreInfoList';
        ...
        //加油券调整后请求接口和参数
        let {stationType, RefuelGsOuCode, RefuelGsOuCodeOp} = this.$route.query
        if(stationType === 'coupon') {
          url = '/app/json/station/getStationList'
          params1.RefuelGsOuCode = RefuelGsOuCode
          params1.RefuelGsOuCodeOp = RefuelGsOuCodeOp
        }
        if (this.mode.length) {
        ...
        this.$http.post(url, params1).then(res => {
          ...
          if (data.status == 0) {
            ...
            // if (this.$store.state.globalConfig.stationSortType == 1 && this.searchText) {
            //   console.log('stationSortType', this.$store.state.globalConfig.stationSortType);
            //   let point1 = new BMap.Point(this.locationinfo.longitude, this.locationinfo.latitude);
            //   this.stations = this.stations.concat(list || []).map(item => {
            //     let point2 = new BMap.Point(item.posx, item.posy);
            //     item.distance = this.bdMap.getDistance(point1, point2)
            //     return item
            //   }).sort((a, b) => {
