    ...
    <template v-if="orderDetail">
      ...
      <div class="detailli">
        <ul>
          ...
          <li v-if="$store.state.globalConfig.appOilPayDetailHideFefuelLiter != 1">
            <span>加油升数</span>
      ...
      <div class="evaluation" v-if="$store.state.globalConfig.oil_appraise_type != 1&&$store.state.globalConfig.oil_appraise_type != 2">
        <span class="text" @click="goEvaluation">去评价</span>
      </div>
      <div v-if="$store.state.globalConfig.oil_appraise_type == 1">
...
<!--&lt;!&ndash;        <p class="evaluate">加油评价</p>&ndash;&gt;-->
<!--        <div class="question" >-->
<!--&lt;!&ndash;          <evaluate :detail="orderDetail"></evaluate>&ndash;&gt;-->
<!--&lt;!&ndash;          <subject :firmdetailSubject="firmdetailSubject" :orderIds="orderId" :ouCode="ouCode" @firmData="firmData"></subject>&ndash;&gt;-->
<!--        </div>-->
<!--      </div>-->
  ...
  export default {
    ...
    computed: {
      ...
      refuelCardNo() {
        ...
        if(this.orderDetail.refuelCardNo){
          return num.slice(-6) || "";
        }else{
          return ''
        }
    ...
    methods: {
      ...
      goEvaluation() {
        ...
        let paramsData = {
          token: this.$store.state.login.token,
          orderId: this.orderDetail.refuelOrderId
        };
        this.$http.post(url, paramsData).then(
          res => {
            this.$Loading.close();
            ...
            //     query: {id: this.configId, orderId: this.orderDetail.refuelOrderId}
            //   })
            // } else {
