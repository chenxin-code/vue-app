    ...
    <div class="top" v-for="(item, index) in answerList" :key="index">
      ...
      <div class="singleList" v-if="item.kindId === 5">
        ...
        <div class="divObj divObj1">
          ...
          <textarea v-if="item.limitMin && item.limitMax" class="textType" v-model="item.result"
                    style="min-height: 90px;" :placeholder="placeHolder?placeHolder:'对其他方面不满意，可描述问题帮加油站改善哦～'"></textarea>
          <textarea v-if="!item.limitMin && !item.limitMax" class="textType" v-model="item.result" :placeholder="placeHolder?placeHolder:'对其他方面不满意，可描述问题帮加油站改善哦～'"
...
<!--          <span class="wall-title">{{item.orderNo}}、{{item.title}}</span>-->
    ...
    <div class="handler" v-if="answerList.length>0">
      <van-button round type="info"  color="#F02C2D"  class="btn" @click="commit" >提交</van-button>
      <div v-if="hasPrize">
        <p v-if="prize.length>0&&prize[0].prizeType!='20030'">* 完成评价后可获赠{{prize[0].prizeName}}</p>
        <p v-if="prize.length>0&&prize[0].prizeType=='20030'">* 完成评价后可获赠{{prize[0].prizeName}}参与次数{{prize[0].prizeNum}}</p>
      </div>
    </div>
...
<script>
  ...
  import api from '@/components/mall2/game/question/api.js'
  export default {
    props:['detail'],
    name: "",
    mixins:[api],
    ...
    methods: {
      ...
      sub() {
        ...
        that.answerList.forEach(function (item) {
          params.push({
            ...
            orderNo: that.detail.marketOrderNo,
        ...
        let par=Object.assign({
          ...
          orderId:that.detail.marketOrderNo,
      ...
      loadData() {
        ...
        let params1 = {
          // id: this.$route.query.id,
          orderId:this.detail.marketOrderNo,
          // refuelOrderId: this.$route.query.refuelOrderId,
        };
        that.$http.post('/app/json/questionaire/getDefaultConfig',params1).then(res => {
          console.log(res);
          if (res.data.result === "success") {
            ...
            this.prize = JSON.parse(res.data.data.questionPrize)
            // let answerList = res.data.data.subjectList;
            let answerList = JSON.parse(res.data.data.subjectJson);
      ...
      getQuest(){
        ...
        let params = {
          id: this.details.id,
          orderId:this.detail.marketOrderNo,
          ouCode:this.detail.storeOuCode
          // refuelOrderId: this.$route.query.refuelOrderId,
          // token: this.$store.state.login.token,
          //随机字符串为了多次答题
        }
