  ...
  export default {
    ...
    methods: {
      ...
      recordItemClick(item) {
        this.$router.push({
          path: '/bind_provincial_record_details', query: {cardInfo: JSON.stringify(item), recordState: this.status}
        })
      },
      getRecordList(status) {
        this.$request.post('/app/json/card/bindCardList', {approvalState: status}).then(res => {
          if (res.status == 0) {
