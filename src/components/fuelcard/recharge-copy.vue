<template>
  <div id="oilMoneyss">
    ...
    <!-- 选择卡号模态框 -->
    <div class="modal" v-if="isshowcard" @click="closetan">
    ...
    <!-- 确定支付模态框 -->
    <div class="modal" v-if="isshowcards">
      <div class="modal-boxs">
        <p>主卡卡号&nbsp;&nbsp;&nbsp;&nbsp; {{cardnumber}}</p>
        <p>用户姓名&nbsp;&nbsp;&nbsp;&nbsp;{{cardname}}</p>
        <p>手机号码&nbsp;&nbsp;&nbsp;&nbsp; {{cardphone}}</p>
      ...
      render() {
        ...
        this.$http.post(url, {}).then(
          res => {
            ...
            _this.cardnumber = res.data.data.cardAcctList[this.findex].cardno; // 卡号
            _this.cardphone = res.data.data.cardAcctList[this.findex].reservedphone;  // 手机号
            _this.cardname = res.data.data.cardAcctList[this.findex].cardholder;  // 姓名
      ...
      rendermode2() {
        ...
        this.$http.post(url, {
          ...
          res => {
            ...
            this.zhilist = res.data.data;
            // this.zhifufang=res.data.data[0].text; //默认充值方式
            this.zhifudizhi = res.data.data[this.indexs].href;  //充值充值地址
...
<style scoped>
  ...
  .chongtop .chongtopBox div {
    border-radius: 8px;
    /*margin-top: 22px;*/
    box-shadow: 0px -5px 0px #f02c2d;
  }

  .chongtop .chongtopBox div p {
    padding-top: 6px;
  }

  .chongtop .chongtopBox p:nth-child(3) {
    border-bottom: 0;
  }
